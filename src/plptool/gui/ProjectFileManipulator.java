...
public class ProjectFileManipulator
{
	public static int CLI(String[] args, int archID, boolean autoloadModules)
	{
		...
		else if (args[2].equals("-a"))
		{
			...
			if (plp.asm != null && plp.asm.isAssembled()
					&& plp.getArch().getStringID().equals("plpmips"))
			{
				...
				Msg.M("Starting address: "
						+ String.format("0x%08x", plp.asm.getAddrTable()[0]));
				plp.save();
			}
		...
		else if (args[2].equals("-p"))
		{
			...
			if (!plp.isDirty())
			{
				...
				return Msg.E("Binary files are not up to date!",
						Constants.PLP_GENERIC_ERROR, plp);
	...
	{
		System.out
				.println("Project file manipulator commands, to be run with -plp <plpfile> [command]:");
		System.out
				.println("  -c <asm 1> <asm 2> ...  Create a new PLP project <plpfile> and import");
		System.out
				.println("                            <asm 1>, <asm 2>, ... into the project");
		System.out
				.println("  -p <port>               Program PLP target board with <plpfile> using serial");
		System.out.println("                            port <port>");
		System.out.println("  -a                      Assemble <plpfile>");
		System.out
				.println("  -i <asm 1> <asm 2> ...  Import <asm 1>, <asm 2>, ... into <plpfile>");
		System.out
				.println("  -d <directory>          Import all files within <directory> into <plpfile>");
		System.out
				.println("  -e <index> <file>       Export source file with <index> as <file>");
		System.out
				.println("  -r <index>              Remove source file with <index> from <plpfile>");
		System.out
				.println("  -s <index>              Set source file with <index> as the main program");
		System.out
				.println("  -m <index> <new index>  Move the source file with <index> to a new position");
