...
/**
 * This is the main class of PLPTool. This class handles command line arguments and sets
 * up the environment for PLPTool execution. This class is responsible for setting up
 * runtime environments (app properties), loading dynamic modules specified in command
 * line arguments, initiating module autoload procedure, and determining next phase of
 * execution (GUI or command line).
 */
public class PLPToolApp extends SingleFrameApplication
{
	...
	protected void startup()
	{
		...
		try
		{
			...
			else
			{
				...
				images.put("__NOT_FOUND__", ImageIO.read(PLPToolApp.class
						.getResource("resources/invalid.png")));
				Msg.D("Creating temporary directory (" + PLPToolbox.getTmpDir() + ")...",
						2, null);
				PLPToolbox.checkCreateTempDirectory();
				// Launch the ProjectDriver
				loadConfig();
				ProjectDriver plp = new ProjectDriver(headless ? Constants.PLP_DEFAULT
						: Constants.PLP_GUI_START_IDE);
	...
	public static void main(String[] args)
	{
		...
		for (int i = 0; i < args.length; i++)
		{
			...
			else if (args.length >= activeArgIndex + 1
					&& args[i].equals("--suppress-output"))
			{
				Msg.silent = true;
				activeArgIndex++;
				...
				// Suppress warnings
					...
					&& args[i].equals("--suppress-warning"))
			{
			...
			else if (args.length >= activeArgIndex + 1 && args[i].equals("-q"))
			{
				...
				// Instantiate project driver and exit. Used by autotest
					...
					&& args[i].equals("--debug-projectdriver"))
			{
			...
			else if (args.length >= activeArgIndex + 3
					&& args[i].equals("--generate-manifest"))
			{
			...
			else if (args.length >= activeArgIndex + 7
					&& args[i].equals("--embed-manifest"))
			{
				Msg.M("Generating manifest for '" + args[i + 1] + "'...");
				String manifest = DynamicModuleFramework.generateManifest(args[i + 1],
				...
				if (manifest != null)
				{
					if (PLPToolbox.addToJar(args[i + 1], "plp.manifest",
							manifest.getBytes()) != Constants.PLP_OK)
					{
					...
					Msg.M("\nMANIFEST GENERATION FAILED, '" + args[i + 1]
							+ "' is unmodified.");
				quit(Constants.PLP_OK);
				...
				// Bring up the embed manifest GUI
					...
					&& args[i].equals("--embed-manifest-gui"))
			{
			...
			{
				Msg.E("Invalid argument: '" + args[i] + "'", Constants.PLP_TOOLAPP_ERROR,
						null);
				System.out
						.println("\nRun with '--full-help' for a complete listing of options.");
	...
	private static void printTerseHelpMessage()
	{
		...
		System.out
				.println("                          Launch PLPTool GUI. PLPTool will open [plpfile]");
		System.out.println("                            if it is provided");
		System.out.println();
		System.out.println("Non-GUI options:");
		System.out.println("  -plp <plpfile> [command]");
		System.out
				.println("                          If no command is specified, print out the list of");
		System.out
				.println("                            source files contained in <plpfile>. This will also");
		System.out
				.println("                            create <plpfile> if it does not exist.");
		System.out
				.println("                            Use '-plp' by itself for command listing");
		System.out
				.println("  -s <plpfile>            Launch the command line simulator to simulate");
		System.out.println("                            <plpfile>");
		System.out
				.println("  -r <plpfile> <script>   Run the simulator in non-interactive mode");
		System.out
				.println(" --headless               Run in headless mode for non-GUI modules");
	}
	...
	private static void printFullHelpMessage()
	{
		...
		System.out
				.println("  --about                 Print licensing information and quit");
		System.out.println("  --gpl                   Print GPL license text and quit");
		System.out
				.println("  --isa-id <arch id>      Force PLPTool to use the ISA with <arch id> for newly");
		System.out.println("                            created projects");
		System.out
				.println("  --remove-config         Remove saved configuration and reset all settings");
		System.out
				.println("  --serialterminal        Launch the serial terminal instead of the IDE");
		...
		System.out
				.println("  --autotest-help         Unit test / automation framework information");
		System.out.println();
		System.out.println("Dynamic modules / extensions controls:");
		System.out
				.println("   -L <jar>               Load a PLPTool module JAR file, locate the manifest,");
		System.out
				.println("                            and interpret the file accordingly. PLPTool will");
		System.out
				.println("                            ONLY launch if the module is successfully loaded");
		System.out
				.println("   -D <path>              Attempt to load all modules in the directory ");
		System.out.println("                            specified by <path>");
		System.out
				.println("   -N                     Do not load any modules at startup");
		System.out
				.println("   -W                     Do not warn about the dangers of loading a module");
		System.out
				.println("   -S <URL>               Fetch a module's jar file from URL, save it to the");
		System.out
				.println("                            the autoload directory, and quit. This module will");
		System.out
				.println("                            be autoloaded the next time PLPTool starts. Module");
		System.out
				.println("                            autoloading can be disabled by the user");
		System.out
				.println("                            (autoloading is enabled by default)");
		System.out
				.println("   -R                     Delete the autoload cache directory and all of its ");
		System.out.println("                            contents");
		System.out
				.println("  --module-debugging      Print a list of module debugging options");
		System.out.println();
	}
	...
	private static void printModuleDebuggingHelpMessage()
	{
		...
		System.out
				.println("                          Load Java class file with the ClassLoader");
		System.out
				.println("  --load-jar <jar>        Load all Java classes inside the specified jar file");
		System.out.println("  --generate-manifest <path or jar> <destination directory>");
		System.out
				.println("                          Generate plp.manifest of Java classes that are in");
		System.out
				.println("                            the specified path. This manifest file will be");
		System.out
				.println("                            written in the destination directory");
		System.out
				.println("  --embed-manifest <jar> <title> <author> <license> <description> <version>");
		System.out
				.println("                          Generate plp.manifest of Java classes that are in");
		System.out
				.println("                            the specified JAR file and embed the manifest in");
		...
		System.out
				.println("   -P<key>::<value>       Pass a key-value property pair to the application");
		System.out.println();
	}
	 ...
	 *
	 * @return HashMap of attribute pairs specified by the user during the start of
	 *         execution
	 */
	...
	/**
	 * Load dynamic modules and apply manifest entries. Should be called just ONCE in a
	 * PLPTool execution path.
	 *
	 * @param plp
	 *            Reference to the ProjectDriver
	 */
	...
	/**
	 * Handle headless execution. Note: modules have to be explicitly included in the
	 * command line via -L or -D. Saved modules and usermods will NOT be automatically
	 * loaded!
	 */
	private static void headless()
	{
		if (!loadModules)
		{
			Msg.E("'-N' can not be used with '--headless'", Constants.PLP_TOOLAPP_ERROR,
					null);
			quit(Constants.PLP_TOOLAPP_ERROR);
		}
		loadConfig();
	...
	public static String getBuildInfo()
	{
		...
		{
			Msg.E("Unable to retrieve build information.", Constants.PLP_GENERIC_ERROR,
					null);
			return ret;
		}
		ret += "This PLPTool package was built on " + prop.getProperty("buildstamp")
				+ " by " + prop.getProperty("builder") + "\n";
	 ...
	 *
	 * @param key
	 *            Image key (filename in JAR with slashes replaced by periods)
	 * @return BufferedImage object of the image. A default image will be returned if the
	 *         key was not found
	 */
	 ...
	 *
	 * @param key
	 *            String key to find the image
	 * @param img
	 *            The image as BufferedImage class
	 */
	...
	/**
	 * Attempt to load configuration from ~/.plp/config. Also checks whether we have
	 * access to a user configuration directory (~/.plp)
	 */
	public static void loadConfig()
	{
		...
		if (!confDir.exists() && !confDir.mkdir())
		{
			Msg.W("Unable to create configuration directory. User settings "
					+ "will not be saved and module functionality will fail!", null);
		}
		...
		if (config.exists() && !config.isDirectory())
		{
			...
			try
			{
				...
				for (int i = 0; i < lines.length; i++)
				{
					...
					{
						Config.cfgAskBeforeAutoloadingModules = Boolean
								.parseBoolean(tokens[1]);
					}
					else if (tokens[0].equals("cfgAskForISAForNewProjects"))
					{
						Config.cfgAskForISAForNewProjects = Boolean
								.parseBoolean(tokens[1]);
					}
					else if (tokens[0].equals("prgAutoDetectPorts"))
					{
						...
						PLPToolApp.getAttributes().put(tokens[0],
	...
	public static void saveConfig()
	{
		...
		if (config != null)
		{
			...
			try
			{
				...
				out.write("cfgAskBeforeAutoloadingModules::"
						+ Config.cfgAskBeforeAutoloadingModules + "\n");
				out.write("cfgAskForISAForNewProjects::"
						+ Config.cfgAskForISAForNewProjects + "\n");
