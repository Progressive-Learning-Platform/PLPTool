...
public class ASMSimView extends javax.swing.JFrame
{
	...
	public final void updateTable()
	{
		...
		for (int i = 0; i < objCode.length; i++)
		{
			...
			dispBinary = String.format("%32s", Long.toBinaryString(objCode[i])).replace(
					" ", "0");
			dispASCII = PLPToolbox.asciiWord(objCode[i]);
			Object[] row = new Object[cols];
			row[0] = (plp.asm.lookupLabel(plp.asm.getAddrTable()[i]) != null ? plp.asm
					.lookupLabel(plp.asm.getAddrTable()[i]) : "");
			row[1] = String.format("0x%08x", plp.asm.getAddrTable()[i])
					+ " "
					+ ((plp.sim != null) ? ((((plptool.mips.SimCore) plp.sim).pc.eval() == plp.asm
							.getAddrTable()[i]) ? "[PC]" : "") : "");
			...
			row[cols - 1] = plp.asm.getAsmList().get(plp.asm.getFileMapper()[i])
					.getAsmFilePath()
	...
	public final void updatePC()
	{
		...
		for (int i = 0; i < objCode.length; i++)
		{
			...
			{
				model.setValueAt(String.format("0x%08x", plp.asm.getAddrTable()[i])
						+ " [PC]", i, Constants.ASMVIEW_ADDR);
			}
			else
			{
				model.setValueAt(String.format("0x%08x", plp.asm.getAddrTable()[i]), i,
						Constants.ASMVIEW_ADDR);
			}
		}
	}
	...
	public void copyTable(int start, int end)
	{
		...
		for (int i = start; i <= end; i++)
		{
			out += !chkIncludeFile.isSelected() ? "" : i != start
					&& source[i - 1].equals(source[i]) ? t + " " : source[i] + " ";
	...
	private void initComponents()
	{
		...
		org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application
				...
				.getResourceMap(ASMSimView.class);
		...
		tblASM.setModel(new javax.swing.table.DefaultTableModel(new Object[][] {
		...
		tblASM.getColumnModel()
				.getSelectionModel()
				.setSelectionMode(
						javax.swing.ListSelectionModel.SINGLE_INTERVAL_SELECTION);
		tblASM.getColumnModel().getColumn(0).setPreferredWidth(100);
		tblASM.getColumnModel().getColumn(0)
				.setHeaderValue(resourceMap.getString("tblASM.columnModel.title1")); // NOI18N
		tblASM.getColumnModel().getColumn(1).setPreferredWidth(100);
		tblASM.getColumnModel().getColumn(1)
				.setHeaderValue(resourceMap.getString("tblASM.columnModel.title0")); // NOI18N
		tblASM.getColumnModel().getColumn(2).setPreferredWidth(350);
		tblASM.getColumnModel().getColumn(2)
				.setHeaderValue(resourceMap.getString("tblASM.columnModel.title4")); // NOI18N
		tblASM.getColumnModel().getColumn(3)
				.setHeaderValue(resourceMap.getString("tblASM.columnModel.title5")); // NOI18N
		tblASM.getColumnModel().getColumn(4)
				.setHeaderValue(resourceMap.getString("tblASM.columnModel.title6")); // NOI18N
		tblASM.getColumnModel().getColumn(5)
				.setHeaderValue(resourceMap.getString("tblASM.columnModel.title7")); // NOI18N
		tblASM.getColumnModel().getColumn(6)
				.setHeaderValue(resourceMap.getString("tblASM.columnModel.title8")); // NOI18N
		tblASM.getColumnModel().getColumn(7).setPreferredWidth(100);
		tblASM.getColumnModel().getColumn(7)
				.setHeaderValue(resourceMap.getString("tblASM.columnModel.title2")); // NOI18N
		tblASM.getColumnModel().getColumn(8).setPreferredWidth(35);
		tblASM.getColumnModel().getColumn(8)
				.setHeaderValue(resourceMap.getString("tblASM.columnModel.title3")); // NOI18N
				...
				.addGroup(
						layout.createSequentialGroup()
								...
								.addPreferredGap(
										javax.swing.LayoutStyle.ComponentPlacement.RELATED,
										331, Short.MAX_VALUE)
								.addComponent(lblInfoMultiword).addContainerGap())
				.addGroup(
						layout.createSequentialGroup()
								...
								.addPreferredGap(
										javax.swing.LayoutStyle.ComponentPlacement.RELATED)
								.addComponent(chkIncludeHeader).addGap(160, 160, 160))
				.addComponent(jScrollPane1, javax.swing.GroupLayout.DEFAULT_SIZE, 797,
						Short.MAX_VALUE));
				...
				layout.createSequentialGroup()
						.addGroup(
								layout.createParallelGroup(
										...
										.addComponent(lblDisplay).addComponent(chkHex)
										.addComponent(chkASCII).addComponent(chkDecimal)
										.addComponent(chkBinary).addComponent(chkSource)
						...
						.addGroup(
								layout.createParallelGroup(
										javax.swing.GroupLayout.Alignment.BASELINE)
										.addComponent(btnCopy).addComponent(btnCopyAll)
										.addComponent(chkIncludeFile)
										.addComponent(chkIncludeLabels)
	...
	private void chkSourceActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_chkSourceActionPerformed
		updateTable();
	}// GEN-LAST:event_chkSourceActionPerformed
	
	private void chkHexActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_chkHexActionPerformed
		updateTable();
	}// GEN-LAST:event_chkHexActionPerformed
	
	private void chkDecimalActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_chkDecimalActionPerformed
		updateTable();
	}// GEN-LAST:event_chkDecimalActionPerformed
	
	private void chkBinaryActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_chkBinaryActionPerformed
		updateTable();
	}// GEN-LAST:event_chkBinaryActionPerformed
	
	private void chkASCIIActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_chkASCIIActionPerformed
		updateTable();
	...
	private void btnCopyAllActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_btnCopyAllActionPerformed
