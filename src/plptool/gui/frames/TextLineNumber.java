...
/**
 * This class will display line numbers for a related text component. The text component
 * must use the same line height for each line. TextLineNumber supports wrapped lines and
 * will highlight the line number of the current line in the text component.
 *
 * This class was designed to be used as a component added to the row header of a
 * JScrollPane.
 */
		...
		PropertyChangeListener, MouseListener
{
	...
	/**
	 * Create a line number component for a text component. This minimum display width
	 * will be based on 3 digits.
	 *
	 * @param component
	 *            the related text component
	 */
	public TextLineNumber(JTextComponent component, TextLineHighlighter tlh,
			ProjectDriver plp)
	{
		this(component, 3);
		this.plp = plp;
	...
	/**
	 * Specify the horizontal alignment of the digits within the component. Common values
	 * would be:
	 * <ul>
	...
	/**
	 * Specify the mimimum number of digits used to calculate the preferred width of the
	 * component. Default is 3.
	 *
	 * @param minimumDisplayDigits
	...
	public void paintComponent(Graphics g)
	{
		...
		while (rowStartOffset <= endOffset)
		{
			try
			{
				...
				if (plp.isSimulating()
						&& plp.sim.breakpoints.hasBreakpoint()
						&& plp.sim.breakpoints.isBreakpoint(plp.getOpenAsm(),
								getLineNumber(rowStartOffset)))
				{
	...
	/*
	 * We need to know if the caret is currently positioned on the line we are about to
	 * paint so the line number can be highlighted.
	 */
	...
	/*
	 * Get the line number to be drawn. The empty string will be returned when a line of
	 * text has wrapped.
	 */
	...
	/*
	 * A document change may affect the number of displayed lines of text. Therefore the
	 * lines numbers will also change.
	 */
		...
		if (e.getClickCount() == 2)
		{
			...
			try
			{
				...
				int endOffset = component
						.viewToModel(new Point(0, component.getHeight()));
				int y;
				int mouse_y;
				int end_y;
				int lineNumber;
				...
				while (rowStartOffset <= endOffset)
				{
					...
					if (mouse_y >= y && mouse_y <= end_y)
					{
						...
						if (addr != -1)
						{
							...
							{
								plptool.Msg.M("New breakpoint set at: "
										+ plp.getAsm(plp.getOpenAsm()).getAsmFilePath()
										+ "(" + lineNumber + "): "
										+ String.format("0x%02x", addr));
								plp.sim.breakpoints.add(addr, plp.getOpenAsm(),
										lineNumber);
				 ...
				 * int line = e.getY() / height + 1; long addr =
				 * plp.asm.getAddrFromFileMetadata(plp.open_asm, line);
				 * 
				 * if(addr != -1) { if(!plp.sim.breakpoints.isBreakpoint(addr)) {
				 * plptool.Msg.M("New breakpoint set at: " +
				 * plp.asms.get(plp.open_asm).getAsmFilePath() + "(" + line + "): " +
				 * String.format("0x%02x", addr)); plp.sim.breakpoints.add(addr,
				 * plp.open_asm, line);
				 * 
				 * } else { plptool.Msg.M("Removing breakpoint.");
				 * plp.sim.breakpoints.remove(addr); } this.repaint(); }
				 */
