...
public class Watcher extends javax.swing.JFrame
{
	...
	public Watcher(ProjectDriver plp)
	{
		...
		this.setIconImage(java.awt.Toolkit.getDefaultToolkit().getImage(
				this.getClass().getResource("resources/toolbar_watcher.png")));
	...
	public static DefaultTableModel getTableInitialModel()
	{
		...
		ret = new javax.swing.table.DefaultTableModel(new Object[][] {
		
		}, new String[] { "Type", "Address", "Hex Value", "Value" }) {
	...
	private void initComponents()
	{
		...
		tblEntries.setModel(new javax.swing.table.DefaultTableModel(new Object[][] {
		...
		tblEntries.getColumnModel().getColumn(0)
				.setHeaderValue(resourceMap.getString("tblEntries.columnModel.title0")); // NOI18N
		tblEntries.getColumnModel().getColumn(1)
				.setHeaderValue(resourceMap.getString("tblEntries.columnModel.title1")); // NOI18N
		tblEntries.getColumnModel().getColumn(2)
				.setHeaderValue(resourceMap.getString("tblEntries.columnModel.title2")); // NOI18N
		tblEntries.getColumnModel().getColumn(3)
				.setHeaderValue(resourceMap.getString("tblEntries.columnModel.title3")); // NOI18N
		...
		cmbValueFormat.setModel(new javax.swing.DefaultComboBoxModel(new String[] {
				...
				layout.createSequentialGroup()
						...
						.addGroup(
								layout.createParallelGroup(
										...
										.addComponent(btnRemoveAll)).addContainerGap()));
	...
	{// GEN-FIRST:event_btnAddActionPerformed
		...
		{
			case 0:
				// if(plp.isAssembled() && (tempAddr =
				// plp.asm.resolveAddress(txtAddr.getText())) != -1)
				if (plp.isAssembled())
				...
				if (plp.sim.bus.isMapped(addr))
				{
					...
					Object[] row = {
							...
							String.format("0x%08x", addr)
									+ ((label != null) ? " [" + label + "]" : ""),
							(data != null) ? String.format("%08x", data & 0xffffffffL)
									: "Uninitialized",
			...
			case 1:
				if (plp.getArch().getID() == plptool.ArchRegistry.ISA_PLPMIPS)
				{
					...
					Byte reg = ((plptool.mips.Asm) plp.asm)
							.getRegisterNumberFromName(txtAddr.getText());
					if (reg != null)
						addr = reg;
					else
						addr = PLPToolbox.parseNum(txtAddr.getText());
					...
					if (addr >= 0 && addr <= mipsSim.regfile.endAddr())
					{
						...
						Object[] row = {
								"Register",
								(reg != null ? txtAddr.getText() : String.format(
										"0x%08x", addr)),
								(data != null) ? String
										.format("%08x", data & 0xffffffffL)
										: "Uninitialized",
								(data != null) ? convertValue(data) : "Uninitialized" };
						entries.addRow(row);
						tblEntries.setModel(entries);
	...
	{// GEN-FIRST:event_btnRemoveSelectedActionPerformed
		DefaultTableModel entries = getTblValues();
	...
	{// GEN-FIRST:event_btnRemoveAllActionPerformed
		DefaultTableModel entries = getTblValues();
	...
	private void txtAddrKeyPressed(java.awt.event.KeyEvent evt)
	{// GEN-FIRST:event_txtAddrKeyPressed
		if (evt.getKeyCode() == java.awt.event.KeyEvent.VK_ENTER)
			btnAddActionPerformed(null);
	}// GEN-LAST:event_txtAddrKeyPressed
	
	private void cmbValueFormatActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_cmbValueFormatActionPerformed
		updateWatcher();
	}// GEN-LAST:event_cmbValueFormatActionPerformed
	
	public final void updateFontSize()
	{
		tblEntries.setFont(new java.awt.Font(plptool.Config.devFont, java.awt.Font.PLAIN,
				plptool.Config.devFontSize));
		tblEntries.setRowHeight(tblEntries.getFontMetrics(tblEntries.getFont())
				.getHeight() + 5);
	}
	...
	public void updateWatcher()
	{
		...
		for (int i = 0; i < entries.getRowCount(); i++)
		{
			...
			if (entries.getValueAt(i, 0).equals("Bus")
					&& plp.sim.bus.isInitialized(PLPToolbox.parseNum(addr)))
			{
				String label = plp.asm.lookupLabel(PLPToolbox.parseNum(addr));
				entries.setValueAt(String.format("0x%08x", PLPToolbox.parseNum(addr))
						+ ((label != null) ? " [" + label + "]" : ""), i, 1);
				Long data = (Long) plp.sim.bus.read(PLPToolbox.parseNum(addr));
				entries.setValueAt(
						(data != null) ? String.format("%08x", data & 0xffffffffL)
								: "Uninitialized", i, 2);
				entries.setValueAt((data != null) ? convertValue(data) : "Uninitialized",
						i, 3);
			}
			else if (entries.getValueAt(i, 0).equals("Register")
					&& plp.getArch().getStringID().equals("plpmips"))
			{
				...
				if (entry.startsWith("$"))
				{
					regAddr = ((plptool.mips.Asm) plp.asm)
							.getRegisterNumberFromName(entry);
				}
				else
	...
	private String convertValue(long data)
	{
		switch (cmbValueFormat.getSelectedIndex())
		{
			...
			case 2:
				return String.format("%32s", Long.toBinaryString(data & 0xffffffffL))
						.replace(' ', '0');
			case 3:
				return String.format("%8s", Long.toBinaryString(data & 0xffL)).replace(
						' ', '0');
			case 4:
				return "" + (char) data;
			case 5:
	...
	class CustomCellTextField extends JTextField
	{
		...
		public CustomCellTextField(ProjectDriver plp)
		{
			...
			addFocusListener(new CellFocusListener(plp));
		}
	}
			...
			else
			{
				...
				else if (type.equals("Register"))
				{
					if (plp.getArch().equals("plpmips"))
					{
						if (addr.startsWith("$"))
						{
							Byte regNum = ((plptool.mips.Asm) plp.asm)
									.getRegisterNumberFromName(addr);
							if (regNum != null)
								address = regNum;
							else
								address = -1;
						...
						if (address != -1)
							((plptool.mips.SimCore) (plp.sim)).regfile.write(address,
									newVal, false);
					}
				}
			}
