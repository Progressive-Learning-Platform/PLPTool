...
public final class Develop extends javax.swing.JFrame
{
	...
	public Develop(ProjectDriver plp)
	{
		...
		treeProject.getSelectionModel().setSelectionMode(
		...
		this.setIconImage(java.awt.Toolkit.getDefaultToolkit().getImage(
				this.getClass().getResource("resources/appicon.png")));
		Msg.P(Text.copyrightString);
		Msg.M("");
	}
	
	/*
	 * Check for develop_recent_# attribute from PLPToolApp and populate the recent open
	 * project submenu
	 */
	private void populateRecentProjectList()
	{
		...
		else
		{
			for (int i = 0; i < 5; i++)
			{
				...
				{
					JMenuItem recentItem = new JMenuItem((i + 1) + " "
							+ attributes.get("develop_recent_" + i));
	...
	/**
	 * Go through the recent project list attributes and update the list if needed with
	 * the given path
	 *
	 * @param path
	 *            Path of project file to add to the list
	 */
	...
	public void updateComponents()
	{
			...
			if (plp.isSimulating())
			{
				...
				SwingUtilities.invokeLater(new Runnable() {
					...
					public void run()
					{
						...
						int yPos = (lineNum - 1)
								* txtEditor.getFontMetrics(txtEditor.getFont())
										.getHeight();
						...
						if (yPos > (viewPortY + scroller.getHeight()) || yPos < viewPortY)
							scroller.getViewport().setViewPosition(
									new Point(0, yPos - scroller.getSize().height / 2));
		...
		catch (Exception e)
		{
			JOptionPane
					.showMessageDialog(
							this,
							"Looks like the GUI is being refreshed too fast!\n"
									+ "Slow down simulation speed or turn off IDE refresh in simulation options to prevent this error.",
							"PLPTool Error", JOptionPane.ERROR_MESSAGE);
	...
	{
		lblSimStat.setText((Config.simFunctional ? "Functional " : "")
				+ "Simulation Mode - " + "Cycles / step: " + Config.simCyclesPerStep
	 ...
	 *
	 * @param commit
	 *            Commit current text to source file
	 */
	...
	{
		java.awt.Font newFont = new java.awt.Font(Config.devFont, java.awt.Font.PLAIN,
				Config.devFontSize);
	...
	public int savePLPFileAs()
	{
		...
		if (retVal == javax.swing.JFileChooser.APPROVE_OPTION)
		{
			...
			String oldFile = plp.plpfile != null ? plp.plpfile.getAbsolutePath() : null;
			plp.plpfile = fOut;
	...
	/**
	 * Convenient method asking the user to save the project (if it's modified) first
	 * before committing to an action that may discard any modifications that have been
	 * made.
	 *
	 * @param action
	 *            A concise description of the user action
	 * @param capAction
	 *            Same with action, but with first letter capitalized
	 * @return 2 if user canceled / closed the dialog, -1 if saving was not necessary, 0
	 *         if user saved the file, 1 if the user did not save
	 */
	public int askSaveFirst(String action, String capAction)
	{
		...
		if (plp.isModified())
		{
			Object[] options = { "Save and " + action, capAction + " without saving",
					"Cancel" };
			int n = javax.swing.JOptionPane.showOptionDialog(this,
					"HALT! The project is modified and you are trying to " + action
	...
	{
		DefaultMutableTreeNode node = (DefaultMutableTreeNode) treeProject
	...
	public void syntaxHighlight(String text, int position)
	{
		if (plp.getArch() != null && plp.getArch().hasSyntaxHighlightSupport())
			plp.getArch().getSyntaxHighlightSupport()
					.syntaxHighlightTextAction(plp, text, position);
	}
	...
	/**
	 * Begin simulation routine. External classes MUST call this method if g_dev is at all
	 * available to the user instead of directly using ProjectDriver's simulate method.
	 */
	...
	/**
	 * End simulation routine. External classes MUST call this method if g_dev is at all
	 * available to the user instead of directly using the ProjectDriver's desimulate
	 * method.
	 */
	public void simEnd()
	{
	...
	/**
	 * Get a reference to a tool checkbox menu. Used for synchronizing between menu items
	 * and toggle buttons on the toolbar
	 *
	 * @param index
	 *            Index of the check box menu
	...
	/**
	 * Get a reference to a toggle button. Used for synchronizing between menu items and
	 * toggle buttons on the toolbar
	 *
	 * @param index
	 *            Index of the toggle button
	...
	/**
	 * Attach listeners to the specified module frame x so it deselects the corresponding
	 * control menu and toggle button when closing
	 *
	 * @param x
	 *            Module frame to attach the listener to
	 ...
	 *
	 * @param v
	 *            True for visibile frame, false otherwise
	 */
	...
	public void simStep()
	{
		...
		CallbackRegistry.callback(CallbackRegistry.SIM_STEP_AGGREGATE,
				Config.simCyclesPerStep);
		for (int i = 0; i < Config.simCyclesPerStep && !breakpoint; i++)
		{
			plp.sim.stepW();
			if (plp.sim.breakpoints.hasBreakpoint()
					&& plp.sim.breakpoints.isBreakpoint(plp.sim.visibleAddr))
			{
				Msg.M("--- breakpoint encountered: "
						+ String.format("0x%02x", plp.sim.visibleAddr));
				breakpoint = true;
			}
		}
	 ...
	 *
	 * @param file
	 *            File name of the source program to go to
	 * @param line
	...
	private void initComponents()
	{
		...
		javax.swing.GroupLayout devMainPaneLayout = new javax.swing.GroupLayout(
				devMainPane);
		devMainPane.setLayout(devMainPaneLayout);
		devMainPaneLayout.setHorizontalGroup(devMainPaneLayout.createParallelGroup(
				javax.swing.GroupLayout.Alignment.LEADING).addComponent(splitterV,
				javax.swing.GroupLayout.DEFAULT_SIZE, 863, Short.MAX_VALUE));
		devMainPaneLayout.setVerticalGroup(devMainPaneLayout.createParallelGroup(
				javax.swing.GroupLayout.Alignment.LEADING).addComponent(splitterV,
		...
		btnSimSwitches
				.setToolTipText(resourceMap.getString("btnSimSwitches.toolTipText")); // NOI18N
		...
		btnSimSevenSegments.setToolTipText(resourceMap
				.getString("btnSimSevenSegments.toolTipText")); // NOI18N
		...
		menuNew.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
		...
		menuOpen.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
		...
		menuOpenRecentProject
				.setText(resourceMap.getString("menuOpenRecentProject.text")); // NOI18N
		...
		menuSaveAs.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
				java.awt.event.KeyEvent.VK_A, java.awt.event.InputEvent.SHIFT_MASK
		...
		menuPrint.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
		...
		menuCopy.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
		...
		menuCut.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
		...
		menuPaste.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
		...
		menuFindAndReplace.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
		...
		menuUndo.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
		...
		menuRedo.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
		...
		menuToolbar.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
				java.awt.event.KeyEvent.VK_T, java.awt.event.InputEvent.ALT_MASK
						| java.awt.event.InputEvent.CTRL_MASK));
		...
		menuClearOutputPane.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
		...
		menuAssemble.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
		...
		menuSimulate.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
		...
		menuProgram.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
				java.awt.event.KeyEvent.VK_F4, java.awt.event.InputEvent.SHIFT_MASK));
		...
		menuQuickProgram.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
		...
		menuDeleteASM.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
		...
		menuRemoveAutoloadDir
				.setText(resourceMap.getString("menuRemoveAutoloadDir.text")); // NOI18N
		...
		menuDynamicModuleManager.setText(resourceMap
				.getString("menuDynamicModuleManager.text")); // NOI18N
		...
		menuSerialTerminal.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
		...
		menuNumberConverter.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
		...
		menuSimStep.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
		...
		menuSimReset.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
		...
		menuSimRun.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
		...
		menuStep1.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
				java.awt.event.KeyEvent.VK_1, java.awt.event.InputEvent.ALT_MASK));
		...
		menuStep2.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
				java.awt.event.KeyEvent.VK_2, java.awt.event.InputEvent.ALT_MASK));
		...
		menuStep3.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
				java.awt.event.KeyEvent.VK_3, java.awt.event.InputEvent.ALT_MASK));
		...
		menuStep4.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
				java.awt.event.KeyEvent.VK_4, java.awt.event.InputEvent.ALT_MASK));
		...
		menuStep5.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
				java.awt.event.KeyEvent.VK_5, java.awt.event.InputEvent.ALT_MASK));
		...
		menuClearBreakpoints.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
		...
		menuSimView.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
				java.awt.event.KeyEvent.VK_C, java.awt.event.InputEvent.SHIFT_MASK
						| java.awt.event.InputEvent.CTRL_MASK));
		...
		menuSimWatcher.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
				java.awt.event.KeyEvent.VK_W, java.awt.event.InputEvent.SHIFT_MASK
						| java.awt.event.InputEvent.CTRL_MASK));
		...
		menuSimControl.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
		...
		menuSimIO.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
		...
		menuSimLEDs.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
		...
		menuSimSwitches.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
		...
		menuSimSevenSegments.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
		...
		menuSimUART.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
				java.awt.event.KeyEvent.VK_4, java.awt.event.InputEvent.CTRL_MASK));
		...
		menuSimVGA.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
				java.awt.event.KeyEvent.VK_5, java.awt.event.InputEvent.CTRL_MASK));
		...
		menuSimPLPID.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
				java.awt.event.KeyEvent.VK_6, java.awt.event.InputEvent.CTRL_MASK));
		...
		menuSimGPIO.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
				java.awt.event.KeyEvent.VK_7, java.awt.event.InputEvent.CTRL_MASK));
		...
		menuExitSim.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
		...
		menuQuickRef.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
	...
	{// GEN-FIRST:event_menuExitActionPerformed
		exit();
	}// GEN-LAST:event_menuExitActionPerformed
	
	private void rootmenuHelpActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_rootmenuHelpActionPerformed
	
	}// GEN-LAST:event_rootmenuHelpActionPerformed
	
	private void menuOpenActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuOpenActionPerformed
		openPLPFile();
	}// GEN-LAST:event_menuOpenActionPerformed
	
	private void menuNewActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuNewActionPerformed
		newPLPFile();
	...
	{// GEN-FIRST:event_menuAboutActionPerformed
		about();
	}// GEN-LAST:event_menuAboutActionPerformed
	
	private void menuSaveActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuSaveActionPerformed
		if (plp.save() == Constants.PLP_FILE_USE_SAVE_AS)
			savePLPFileAs();
	}// GEN-LAST:event_menuSaveActionPerformed
	
	private void menuAssembleActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuAssembleActionPerformed
		Msg.setOutput(txtOutput);
	...
	private void menuAssembleActionPerformed1(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuAssembleActionPerformed1
		assemble();
	}// GEN-LAST:event_menuAssembleActionPerformed1
	
	private void menuSaveAsActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuSaveAsActionPerformed
		savePLPFileAs();
	}// GEN-LAST:event_menuSaveAsActionPerformed
	
	private void menuImportASMActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuImportASMActionPerformed
		importASM();
	}// GEN-LAST:event_menuImportASMActionPerformed
	
	private void menuDeleteASMActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuDeleteASMActionPerformed
		deleteASM();
	}// GEN-LAST:event_menuDeleteASMActionPerformed
	
	private void rootmenuProjectActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_rootmenuProjectActionPerformed
	}// GEN-LAST:event_rootmenuProjectActionPerformed
	
	private void menuSetMainProgramActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuSetMainProgramActionPerformed
		if (plp.getOpenAsm() != 0)
			plp.setMainAsm(plp.getOpenAsm());
	}// GEN-LAST:event_menuSetMainProgramActionPerformed
	
	private void btnAssembleActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_btnAssembleActionPerformed
		assemble();
	}// GEN-LAST:event_btnAssembleActionPerformed
	
	private void menuExportASMActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuExportASMActionPerformed
		exportASM();
	...
	private void menuProgramActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuProgramActionPerformed
		plp.g_prg.setVisible(true);
	}// GEN-LAST:event_menuProgramActionPerformed
	
	private void menuNewASMActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuNewASMActionPerformed
	...
	{// GEN-FIRST:event_menuCopyActionPerformed
		txtEditor.copy();
	}// GEN-LAST:event_menuCopyActionPerformed
	
	private void menuCutActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuCutActionPerformed
		// undoManager.modify("delete", "", txtEditor.getSelectionStart(),
		// txtEditor.getSelectionEnd(), txtEditor.getSelectionStart(),
	...
	{// GEN-FIRST:event_menuPasteActionPerformed
		txtEditor.paste();
	...
	private void txtEditorCaretPositionChanged(java.awt.event.InputMethodEvent evt)
	{// GEN-FIRST:event_txtEditorCaretPositionChanged
	
	}// GEN-LAST:event_txtEditorCaretPositionChanged
	
	private void treeProjectMousePressed(java.awt.event.MouseEvent evt)
	{// GEN-FIRST:event_treeProjectMousePressed
		if (evt.getClickCount() == 2)
		{ // user double clicked the project tree
			DefaultMutableTreeNode node = (DefaultMutableTreeNode) treeProject
					.getLastSelectedPathComponent();
			...
			if (node.isLeaf())
			{
				...
				if (nodeStr.endsWith("asm"))
				{
					...
					plp.refreshProjectView(false);
					safeRefresh(false);
					// Msg.M("Going back to line #" +
					// plp.getAsm(plp.getOpenAsm()).getLastLine());
	...
	private void txtEditorCaretUpdate(javax.swing.event.CaretEvent evt)
	{// GEN-FIRST:event_txtEditorCaretUpdate
		if (plp.getAsms() != null && plp.getAsms().size() > 0)
		{
			...
			txtCurFile.setText(fName + ":" + line
					+ (plp.getOpenAsm() == 0 ? " <main program>" : ""));
			
			if (plp.isSimulating())
			{
				long addr = plp.asm.getAddrFromFileMetadata(plp.getOpenAsm(), line);
				if (addr != -1)
					txtCurFile.setText(txtCurFile.getText() + " "
							+ String.format("0x%02x", addr));
			}
		}
	}// GEN-LAST:event_txtEditorCaretUpdate
	
	private void treeProjectMouseClicked(java.awt.event.MouseEvent evt)
	{// GEN-FIRST:event_treeProjectMouseClicked
	
	}// GEN-LAST:event_treeProjectMouseClicked
	
	private void rootmenuProjectMouseClicked(java.awt.event.MouseEvent evt)
	{// GEN-FIRST:event_rootmenuProjectMouseClicked
	}// GEN-LAST:event_rootmenuProjectMouseClicked
	
	private void txtEditorMousePressed(java.awt.event.MouseEvent evt)
	{// GEN-FIRST:event_txtEditorMousePressed
	...
	private void txtEditorKeyTyped(java.awt.event.KeyEvent evt)
	{// GEN-FIRST:event_txtEditorKeyTyped
		if (Config.devNewSyntaxHighlightStrategy)
			return;
		deleteOccured = false;
		...
		else if ((int) evt.getKeyChar() == 10 || (int) evt.getKeyChar() > 31
				&& (int) evt.getKeyChar() < 127)
		{
			deleteOccured = (txtEditor.getSelectedText() != null)
					|| (txtEditor.getSelectedText() != null && !txtEditor
			...
			SwingUtilities.invokeLater(new Runnable() {
				...
				public void run()
				{
							...
							&& !undoManager.isBusy())
					{
						int caretPos = txtEditor.getCaretPosition();
						setHighlighting(true);
						syntaxHighlight(txtEditor.getText().substring(0, caretPos)
								.split("\\r?\\n").length - 1);
						setHighlighting(false);
	...
	private void menuQuickProgramActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuQuickProgramActionPerformed
		plp.g_prg.program();
		plp.g_prg.setVisible(true);
	}// GEN-LAST:event_menuQuickProgramActionPerformed
	
	private void btnProgramActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_btnProgramActionPerformed
		menuProgramActionPerformed(null);
	}// GEN-LAST:event_btnProgramActionPerformed
	
	private void menuQuickRefActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuQuickRefActionPerformed
		plp.showQuickRef();
	}// GEN-LAST:event_menuQuickRefActionPerformed
	
	private void menuManualActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuManualActionPerformed
	...
	private void menuIssuesActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuIssuesActionPerformed
		Desktop desktop = Desktop.getDesktop();
		if (desktop.isSupported(Desktop.Action.BROWSE))
		{
			try
	...
	private void menuIssuesPageActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuIssuesPageActionPerformed
		Desktop desktop = Desktop.getDesktop();
		if (desktop.isSupported(Desktop.Action.BROWSE))
		{
			try
	...
	private void menuFindAndReplaceActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuFindAndReplaceActionPerformed
	...
	{// GEN-FIRST:event_menuOptionsActionPerformed
		if (!plp.isSimulating())
	...
	private void menuSerialTerminalActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuSerialTerminalActionPerformed
		if (plp.term == null)
	...
	{// GEN-FIRST:event_menuPrintActionPerformed
		try
		{
			txtEditor.print();
		}
	...
	private void menuSimStepActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuSimStepActionPerformed
		simStep();
	}// GEN-LAST:event_menuSimStepActionPerformed
	
	private void menuExitSimActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuExitSimActionPerformed
		simEnd();
		// plp.refreshProjectView(false);
		safeRefresh(false);
	}// GEN-LAST:event_menuExitSimActionPerformed
	
	private void menuSimRunActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuSimRunActionPerformed
	...
	private void menuSimIOActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuSimIOActionPerformed
	...
	private void menuSimWatcherActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuSimWatcherActionPerformed
	...
	private void menuSimViewActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuSimViewActionPerformed
	...
	private void menuSimResetActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuSimResetActionPerformed
		simReset();
	}// GEN-LAST:event_menuSimResetActionPerformed
	
	private void menuSimLEDsActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuSimLEDsActionPerformed
		setLEDsFrame(menuSimLEDs.isSelected());
	}// GEN-LAST:event_menuSimLEDsActionPerformed
	
	private void menuSimSwitchesActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuSimSwitchesActionPerformed
		setSwitchesFrame(menuSimSwitches.isSelected());
	}// GEN-LAST:event_menuSimSwitchesActionPerformed
	
	private void menuSimSevenSegmentsActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuSimSevenSegmentsActionPerformed
		setSevenSegmentsFrame(menuSimSevenSegments.isSelected());
	}// GEN-LAST:event_menuSimSevenSegmentsActionPerformed
	
	private void menuSimUARTActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuSimUARTActionPerformed
		setUARTFrame(menuSimUART.isSelected());
	}// GEN-LAST:event_menuSimUARTActionPerformed
	
	private void menuSimVGAActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuSimVGAActionPerformed
		setVGAFrame(menuSimVGA.isSelected());
	}// GEN-LAST:event_menuSimVGAActionPerformed
	
	private void menuSimPLPIDActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuSimPLPIDActionPerformed
		setPLPIDFrame(menuSimPLPID.isSelected());
	}// GEN-LAST:event_menuSimPLPIDActionPerformed
	
	private void btnSimulateActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_btnSimulateActionPerformed
	...
	{// GEN-FIRST:event_btnSimRunActionPerformed
	...
	private void btnSimStepActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_btnSimStepActionPerformed
		simStep();
	}// GEN-LAST:event_btnSimStepActionPerformed
	
	private void btnSimResetActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_btnSimResetActionPerformed
		simReset();
	}// GEN-LAST:event_btnSimResetActionPerformed
	
	private void menuStep1ActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuStep1ActionPerformed
	...
	private void menuStep2ActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuStep2ActionPerformed
	...
	private void menuStep3ActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuStep3ActionPerformed
	...
	private void menuStep4ActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuStep4ActionPerformed
	...
	private void menuStep5ActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuStep5ActionPerformed
	...
	private void menuClearBreakpointsActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuClearBreakpointsActionPerformed
		clearBreakpoints();
	}// GEN-LAST:event_menuClearBreakpointsActionPerformed
	
	private void menuToolbarActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuToolbarActionPerformed
		toolbar.setVisible(menuToolbar.isSelected());
	}// GEN-LAST:event_menuToolbarActionPerformed
	
	private void btnSimLEDsActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_btnSimLEDsActionPerformed
		setLEDsFrame(btnSimLEDs.isSelected());
	}// GEN-LAST:event_btnSimLEDsActionPerformed
	
	private void btnSimSwitchesActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_btnSimSwitchesActionPerformed
		setSwitchesFrame(btnSimSwitches.isSelected());
	}// GEN-LAST:event_btnSimSwitchesActionPerformed
	
	private void btnSimSevenSegmentsActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_btnSimSevenSegmentsActionPerformed
		setSevenSegmentsFrame(btnSimSevenSegments.isSelected());
	}// GEN-LAST:event_btnSimSevenSegmentsActionPerformed
	
	private void btnSimUARTActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_btnSimUARTActionPerformed
		setUARTFrame(btnSimUART.isSelected());
	}// GEN-LAST:event_btnSimUARTActionPerformed
	
	private void btnSimVGAActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_btnSimVGAActionPerformed
		setVGAFrame(btnSimVGA.isSelected());
	}// GEN-LAST:event_btnSimVGAActionPerformed
	
	private void btnSimPLPIDActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_btnSimPLPIDActionPerformed
		setPLPIDFrame(btnSimPLPID.isSelected());
	}// GEN-LAST:event_btnSimPLPIDActionPerformed
	
	private void menuSimControlActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuSimControlActionPerformed
	...
	private void btnWatcherActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_btnWatcherActionPerformed
	...
	{// GEN-FIRST:event_btnCPUActionPerformed
	...
	private void menuSimAsmViewActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuSimAsmViewActionPerformed
		...
		else
		{
			...
			if (plp.isAssembled())
			{
				...
				Msg.E("The project must be assembled first.",
						Constants.PLP_GENERIC_ERROR, null);
		}
	}// GEN-LAST:event_menuSimAsmViewActionPerformed
	
	private void menuProjectPaneActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuProjectPaneActionPerformed
		boolean v;
		...
		{
			hPaneSavedProportion = splitterH.getDividerLocation()
					/ (double) splitterH.getWidth();
	...
	private void menuOutputPaneActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuOutputPaneActionPerformed
		boolean v;
		...
		{
			vPaneSavedProportion = splitterV.getDividerLocation()
					/ (double) splitterV.getHeight();
	...
	private void menuNumberConverterActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuNumberConverterActionPerformed
		if (plp.nconv == null)
	...
	private void menuSimulateActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuSimulateActionPerformed
	...
	private void menuClearOutputPaneActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuClearOutputPaneActionPerformed
		txtOutput.setText("");
	}// GEN-LAST:event_menuClearOutputPaneActionPerformed
	
	private void btnSimControlActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_btnSimControlActionPerformed
	...
	private void menuDynamicModuleManagerActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuDynamicModuleManagerActionPerformed
		(new DynamicModuleManager(this, true, plp)).setVisible(true);
	}// GEN-LAST:event_menuDynamicModuleManagerActionPerformed
	
	private void menuLoadModuleJarActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuLoadModuleJarActionPerformed
		final javax.swing.JFileChooser fc = new javax.swing.JFileChooser();
		...
		if (retVal == javax.swing.JFileChooser.APPROVE_OPTION)
		{
			...
			if (manifest != null)
			{
				...
				if (retVal == JOptionPane.YES_OPTION)
					PLPToolbox.downloadJARForAutoload("file:///"
							+ fc.getSelectedFile().getAbsolutePath(), null, false);
			}
		}
	}// GEN-LAST:event_menuLoadModuleJarActionPerformed
	
	private void btnSimGPIOActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_btnSimGPIOActionPerformed
		setGPIOFrame(btnSimGPIO.isSelected());
	}// GEN-LAST:event_btnSimGPIOActionPerformed
	
	private void menuSimGPIOActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuSimGPIOActionPerformed
		setGPIOFrame(menuSimGPIO.isSelected());
	}// GEN-LAST:event_menuSimGPIOActionPerformed
	
	private void menuRemoveAutoloadDirActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuRemoveAutoloadDirActionPerformed
		int ret = JOptionPane.showConfirmDialog(this,
				"WARNING! This will remove all saved modules. Are you sure?",
				"Clear Module Auto-load Cache", JOptionPane.YES_NO_OPTION,
	...
	private void menuModuleManagerActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuModuleManagerActionPerformed
		(new ModuleManager(this, true, plp)).setVisible(true);
	}// GEN-LAST:event_menuModuleManagerActionPerformed
	
	private void menuSetNewProjectISAActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_menuSetNewProjectISAActionPerformed
		plp.g_isaselect.setLocationRelativeTo(this);
		plp.g_isaselect.populateISASelector();
		plp.g_isaselect.setVisible(true);
	...
	{
		return plp.plpfile != null ? "Develop(" + plp.plpfile.getName() + ")" : "Develop";
	}
...
class DevEditorDocListener implements DocumentListener
{
	...
	{
		if (g_dev.getUndoManager().isBusy() || !enable || g_dev.isHighlighting())
			return;
