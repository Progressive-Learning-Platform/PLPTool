...
/**
 * This is the PLPTool application project backend. This class handles manipulation of the
 * PLP project file and drives the workflow of a PLP tool user. This includes creating,
 * opening, and saving project files. Driving workflow functions such as assembling,
 * simulating, and programming the board.
 *
 * @author wira
 */
public final class ProjectDriver
{
	...
	/*
	 * These variables hold project status and data for this driver modified - denotes
	 * whether the plpfile is modified since opening open_asm - current open ASM file in
	 * the gui arch - active ISA for this project sim_mode - denotes whether the project
	 * is in simulation mode replay - denotes whether the project is in replay mode
	 * asm_req - denotes whether the project needs to be assembled for other functions
	 * such as simulation or programming asms - list of source files halt - --unused for
	 * now--
	 */
	...
	/*
	 * These variables hold data and information loaded from the plp project file.
	 */// --
	...
	/**
	 * The assembler attached to the project. This assembler will most likely be null if
	 * the project is not assembled. It is advised to wrap code segments that access this
	 * handle with an if block checking for isAssembled() method.
	 */
	public PLPAsm asm;
	...
	public ProjectDriver(int modes)
	{
		...
		catch (UnsatisfiedLinkError e)
		{
			...
			Msg.W(" - If you are running Linux, make sure that RXTX library is installed.",
					null);
		...
		if (g && !applet)
		{
			...
			java.awt.Dimension screenResolution = java.awt.Toolkit.getDefaultToolkit()
					.getScreenSize();
			...
			if (X < 0 || Y < 0 || W < 0 || H < 0)
			{
				this.g_dev
						.setSize(
								(int) (Config.relativeDefaultWindowWidth * screenResolution.width),
	...
	public int setArch(int archID)
	{
		...
		if (arch == null)
		{
			...
			return Msg.E("Invalid ISA ID: " + archID + ". Defaulting to "
					+ "plpmips (id 0).", Constants.PLP_ISA_INVALID_ARCHITECTURE_ID, this);
		}
		else
		{
	...
	/**
	 * Initialize plp project data structures. This function provides a default empty
	 * source file and calls the architecture newProject() method.
	 *
	 * @param archID
	 *            The ID of the ISA to use
	 * @return PLP_OK
	 */
	...
	/**
	 * Initialize project data structures and attempt to import the specified assembly
	 * source file into the project.
	 *
	 * @param asmPath
	 *            Path to ASM file to import
	 * @param archID
	 *            The ID of the ISA to use
	 * @return PLP_OK
	 */
	...
	public int save()
	{
		...
		try
		{
			...
			str += "simAllowExecutionOfArbitraryMem::"
					+ Config.simAllowExecutionOfArbitraryMem + "\n";
			str += "simBusReturnsZeroForUninitRegs::"
					+ Config.simBusReturnsZeroForUninitRegs + "\n";
			str += "simDumpTraceOnFailedEvaluation::"
					+ Config.simDumpTraceOnFailedEvaluation + "\n";
	...
	public int open(String path, boolean assemble)
	{
		...
		try
		{
			...
			TarArchiveInputStream tIn = new TarArchiveInputStream(new FileInputStream(
					plpFile));
			...
			while ((entry = tIn.getNextTarEntry()) != null)
			{
				if (entry.getName().equals("plp.metafile"))
				{
					...
					if (metaScanner.findWithinHorizon("ARCH=", 0) != null)
					{
						...
						if (Config.cfgOverrideISA >= 0)
						{ // ISA ID override, ignore the metafile
							arch = ArchRegistry.getArchitecture(this,
									Config.cfgOverrideISA);
							arch.init();
						}
						...
						else
						{
							...
							if (arch == null)
							{
								Msg.W("Invalid ISA ID is specified in the project file: '"
										+ temp + "'. Assuming PLPCPU.", this);
								arch = ArchRegistry.getArchitecture(this,
										ArchRegistry.ISA_PLPMIPS);
							}
							arch.init();
			...
			while ((entry = tIn.getNextTarEntry()) != null)
			{
				...
				handled = CallbackRegistry.callback(CallbackRegistry.PROJECT_OPEN_ENTRY,
						eParams) || handled;
				
				if (entry.getName().endsWith("asm")
						&& !entry.getName().startsWith("plp."))
				{
	...
	public int updateWindowTitle()
	{
		...
		String windowTitle = plpfile.getName() + ((modified) ? "*" : "")
				+ (sim_mode ? " - Simulation Mode " : "") + " - PLP Software Tool "
				+ Text.versionString;
	...
	/**
	 * GUI: update the develop window to reflect the current state of the project driver
	 * (open file, etc.)
	 *
	 * @param commitCurrentAsm
	 *            Whether to commit currently open asm file in the editor before
	 *            refreshing.
	 * @return PLP_OK
	 */
	public int refreshProjectView(boolean commitCurrentAsm)
	{
		...
		root.add(metaRoot);
		for (int i = 0; i < asms.size(); i++)
			srcRoot.add(new DefaultMutableTreeNode(i + ": "
					+ asms.get(i).getAsmFilePath()));
		...
		metaRoot.add(new DefaultMutableTreeNode("ISA="
				+ ArchRegistry.getStringID(arch.getID())));
	...
	/**
	 * Assemble the source files attached to this project, starting from the main program
	 * (index 0)
	 *
	 * @return PLP_OK on successful operation, error code otherwise
	 */
	...
	/**
	 * Notify the project driver that the project will need to be assembled prior to any
	 * simulation or programming operations
	 */
	...
	public int simulate()
	{
		...
		if (!arch.hasSimCore())
			return Msg.E("simulate: This ISA does not implement a simulation" + " core.",
					Constants.PLP_ISA_NO_SIMCORE, this);
		
		if (asm_req)
		{
			int ret = assemble();
			if (ret != Constants.PLP_OK)
				return Constants.PLP_ASM_ASSEMBLE_FAILED;
		}
	...
	/**
	 * Program the board with the object code produced by the assembler to the specified
	 * port
	 *
	 * @param port
	 *            Programming port to use
	 * @return PLP_OK on successful operation, error code otherwise
	 */
	public int program(String port)
	{
		...
		try
		{
			...
			if (asm != null && asm.isAssembled())
			{
				...
				if (asm.getObjectCode().length < 1)
					return Msg.E("Empty program.", Constants.PLP_PRG_EMPTY_PROGRAM, this);
				
				prg = arch.createProgrammer();
				
				if (prg == null)
					return Msg.E("The specified ISA does not have the serial "
							+ "programmer implemented.", Constants.PLP_ISA_NO_PROGRAMMER,
							this);
				...
				if (arch.equals("plpmips") && Config.prgNexys3ProgramWorkaround
		...
		{
			Msg.trace(e);
			return Msg.E("Programming failed.\n" + e, Constants.PLP_GENERIC_ERROR, this);
		}
	}
	
	/**
	 * Alert the project driver that the project state has been changed and may need
	 * saving.
	 */
	 ...
	 *
	 * @param index
	 *            Index of the source file to open and display in editor
	 */
	 ...
	 *
	 * @param asms
	 *            ArrayList of references to source files
	 */
	 ...
	 *
	 * @param index
	 *            Index of the source file to update
	 * @param newStr
	 *            New string to store
	 * @return PLP_OK on successful operation, error code otherwise
	 */
	...
	public int importAsm(String path)
	{
		...
		for (int i = 0; i < asms.size(); i++)
		{
			...
			if (existingPath.equals(path) || existingPath.equals(asmFile.getName()))
			{
				return Msg.E("importAsm(" + path
	...
	/**
	 * Export the source file specified by index and save it to the specified output path.
	 *
	 * @param index
	 *            Index of the source file to export
	 * @param path
	 *            The path of export destination
	 * @return PLP_OK on successful operation, error code otherwise
	 */
	public int exportAsm(int index, String path)
	{
		...
		Msg.I("Exporting " + asms.get(index).getAsmFilePath() + " to " + path, null);
	 ...
	 *
	 * @param asmFileName
	 *            Name of the source file to remove
	 * @return PLP_OK on successful operation, error code otherwise
	 */
	public int removeAsm(String asmFileName)
	{
		...
		return Msg.E("removeAsm: Can not find source file with the name \"" + asmFileName
				+ "\" within the project.", Constants.PLP_BACKEND_ASM_DOES_NOT_EXIST,
				this);
	}
	
	/**
	 * Move the specified source file to index 0, making it the top level source file.
	 *
	 * @param index
	 *            Index of the source file to be set as main program
	 * @return PLP_OK on successful operation, error code otherwise
	 */
	 ...
	 *
	 * @param index
	 *            Index of the source file to be modified
	 * @param newIndex
	 *            The new position of the source file
	 * @return PLP_OK on successful operation, error code otherwise
	 */
	...
	public void updateComponents(boolean updateDevelop)
	{
		...
		catch (Exception e)
		{
			...
			System.out
					.println("GUI error has occured. Switch to debug level 2 or above to print stack trace.");
			Msg.trace(e);
		}
	}
