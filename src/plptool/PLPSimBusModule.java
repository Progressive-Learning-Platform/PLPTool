...
/**
 * This is an abstract for a general purpose module that can be attached to the front-side
 * bus. Registers for the module are implemented using TreeMap. This class provides all
 * the necessary functions to implement a PLP simulator bus module, such as read and write
 * functions, and address space management and error checking. The module then can be
 * extended by implementing eval() and gui_eval() functions, effectively tying the module
...
{
	/**
	 * Denotes whether this module should run on its own thread. Defaults to false.
	 */
	...
	/**
	 * Denotes each entry in the register whether it is an instruction. Useful for main
	 * memory implementations or caches.
	 */
	...
	/**
	 * Write-enable and whether evaluations are enabled, but subclasses can choose to
	 * ignore this
	 */
	protected boolean enabled;
	
	/**
	 * This variable tells the bus whether it actually stores data in the mapped region.
	 * Defaults to false. Useful for modules that need to see bus accesses but do not
	 * require registers (cache simulators, snoopers, etc)
	 */
	public boolean phantom;
	
	/**
	 * PLPSimBusModule defaults to returning the current version of PLPTool. User modules
	 * can override this. May be useful for dynamic modules
	 * 
	...
	/**
	 * The constructor for the superclass requires the address space and whether the
	 * registers of the module are word-aligned
	 *
	 * @param startAddr
	 *            Starting address of the module's address space
	 * @param endAddr
	 *            End address of the module's address space
	 * @param wordAligned
	 *            Whether the module's address space is word aligned
	 */
	...
	/**
	 * Write data to one of the module's registers. Only possible when the module is
	 * enabled. The write command is ignored otherwise.
	 *
	 * @param addr
	 *            Address to write to
	 * @param data
	 *            Data to be written
	 * @param isInstr
	 *            Denotes whether the value to be written is an instruction
	...
	public synchronized final int writeReg(long addr, Object data, boolean isInstr)
	{
		if (addr > endAddr || addr < startAddr)
			...
			return Msg.E("write(" + String.format("0x%08x", addr)
					+ "): Requested address is unaligned.",
					Constants.PLP_SIM_OUT_UNALIGNED_MEMORY, this);
		else if (!enabled)
			return Msg.E("write(" + String.format("0x%08x", addr)
					+ "): Module is disabled.", Constants.PLP_SIM_MODULE_DISABLED, this);
		else
		{
	...
	/**
	 * Wrapper function for writeReg. Developers may wish to override this function to
	 * implement module-specific write actions without sacrificing PLPSimBusModule
	 * original writeReg functionality. This is the actual function that will be called by
	 * PLPSimBus.
	 * 
	 * @param addr
	 *            Address to write to
	...
	/**
	 * Wrapper function for readReg. Developers may wish to override this function to
	 * implement module-specific read actions without sacrificing PLPSimBusModule original
	 * readReg functionality. This is the actual function that will be called by
	 * PLPSimBus.
	 *
	 * @param addr
	 *            Address to read from
	 ...
	 * file on evaluation.
	 *
	 * @return n x 3 Object array containing addresses, values of the memory module and
	 *         boolean denoting whether the value is instruction or not.
	 */
	...
	/**
	 * The eval() function represents the behavior of the module itself. For example, an
	 * eval function for an array of LED will read its register and light up proper LEDs.
	 * Simulation cores will call this function every cycle, so this may slow down the
	...
	/**
	 * gui_eval is designed for simulator developers / users to allow the module to
	 * interact with the simulation environment directly. This function is only called
	 * every time the GUI components are refreshed. Simulation cores do not/should not
	 * call this function every cycle.
	 *
	 * @param x
	 *            Reference to a frame object that this module will interact with.
	 * @return Status code.
	 */
	abstract public int gui_eval(Object x);
	
	/**
	 * This method is called by PLPSimBus.reset(). It is up to the module on how this
