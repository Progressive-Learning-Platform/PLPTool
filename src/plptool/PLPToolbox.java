...
public class PLPToolbox
{
	...
	/**
	 * Parse a hexadecimal or binary number into unsigned long, but stay silent if parsing
	 * failed.
	 *
	 * @param number
	 *            Hex or binary number
	...
	/**
	 * Try if the given label resolves to an address. If not, parse the string as an
	 * address.
	 *
	 * @param label
	 *            Label to resolve
	 * @param asm
	 *            Assembly object
	 * @return The address of label if the label resolves, if not, the address if it
	 *         parses as an address, -1 otherwise
	 */
	 ...
	 *
	 * @param print
	 *            Print host OS information to message out
	...
	{
		return (getOS(false) == Constants.PLP_OS_LINUX_32 || getOS(false) == Constants.PLP_OS_LINUX_64);
	}
	
	/**
	 * Attach a keylistener that will hide the frame when the escape key is pressed by the
	 * user
	 *
	 * @param frame
	 *            Reference to the frame to attach the listener to
	 */
	...
	/**
	 * Attach a keylistener that will hide the dialog when the escape key is pressed by
	 * the user
	 *
	 * @param frame
	 *            Reference to the dialog to attach the listener to
	 */
	 ...
	 *            Reference to the frame to attach the listener to
	 */
	public static void attachDebugConsoleMagicComboListener(
			final javax.swing.JFrame frame, final plptool.gui.ProjectDriver plp,
			final boolean show)
	{
	...
	/**
	 * Attempt to download a JAR file from an URL. Also provides an option of loading and
	 * applying the module's manifest immediately after.
	 *
	 * @param URL
	 *            Location of the JAR file
	 * @param plp
	 *            Reference to the ProjectDriver instance
	 * @param load
	 *            Should the method load the module after downloading
	 */
	public static boolean downloadJARForAutoload(String URL,
			plptool.gui.ProjectDriver plp, boolean load)
	{
		...
		if (plp == null || !plp.g() || ret == javax.swing.JOptionPane.YES_OPTION)
		{
			...
			try
			{
				...
				fos.close();
				if (!DynamicModuleFramework.checkForManifest(PLPToolbox.getConfDir()
						+ "/autoload/" + fileName))
				{
					Msg.E("Downloaded JAR file does not contain plp.manifest",
							Constants.PLP_DMOD_NO_MANIFEST_FOUND, null);
					(new File(PLPToolbox.getConfDir() + "/autoload/" + fileName))
							.delete();
					return false;
				}
			}
			...
			{
				Msg.E("Failed to fetch " + URL + ".", Constants.PLP_GENERIC_ERROR, null);
			...
			if (load)
			{
				...
				String[] manifest = DynamicModuleFramework
						.loadJarWithManifest(searchPath);
				if (manifest != null)
					DynamicModuleFramework.applyManifestEntries(PLPToolbox.getConfDir()
							+ "/autoload/" + fileName, manifest, plp);
			}
			return true;
		}
		return false;
	 ...
	 *
	 * @param jar
	 *            Path to JAR file to extract the file from
	 * @param entry
	 *            Name of the file to extract
	 * @param dest
	 *            Destination path to write the file to
	 * @return PLP_OK on successful copy, error code otherwise
	 */
	public static int copyFromJar(String jar, String entry, String dest)
	{
		try
		{
			...
			if (jarEntry == null)
				return Msg.E("copyFromJar: " + "Can not find entry: '" + entry + "' in '"
						+ jar + "'.", Constants.PLP_GENERAL_IO_ERROR, null);
		...
		{
			return Msg.E("copyFromJar: " + "Jar extract error: '" + entry + "' to '"
					+ dest + "' from '" + jar + "'."
					+ (Constants.debugLevel >= 2 ? "Exception: " + e : ""),
					Constants.PLP_GENERAL_IO_ERROR, null);
		}
	...
	public static int createJar(String jar, String dirPath)
	{
		...
		{
			return Msg.E("I/O error while trying to archive '" + dir.getAbsolutePath()
					+ "' into '" + jar + "'", Constants.PLP_GENERAL_IO_ERROR, null);
		}
	...
	/**
	 * Helper method for the createJar method. This method will recurse into directories
	 * and add entries to the JAR file. Entries are formatted with UNIX-style path
	 * separator (forward slashes) for compatibility with PLPTool class loader
	 *
	 * @param out
	 *            JAR output stream
	 * @param dir
			...
			for (int i = 0; i < files.length; i++)
			{
				...
				else
				{
					try
					{
						...
						entryPath = files[i].getAbsolutePath()
								.replace(root.getAbsolutePath(), "").substring(1);
					...
					{
						return Msg.E(
								"I/O error while adding '" + files[i].getAbsolutePath()
										+ "' to JAR archive.",
								Constants.PLP_GENERAL_IO_ERROR, null);
					}
				}
	 ...
	 * 
	 * @param jar
	 *            Jar file to add entry to (will be created if it doesn't exist)
	 * @param entryPath
	...
	public static int addToJar(String jar, String entryPath, byte[] data)
	{
		...
		{
			Msg.trace(e);
			return Msg.E("Zip Exception: " + e.getMessage(),
					Constants.PLP_GENERAL_IO_ERROR, null);
		}
		catch (IOException e)
		{
	...
	public static void checkCreateTempDirectory()
	{
		...
		{
			Msg.E("Temporary directory creation failed.", Constants.PLP_GENERAL_IO_ERROR,
					null);
		}
	}
	 ...
	 *
	 * @param map
	 *            Reference to the map object to convert
	 * @return 2-dimensional array with keys in the first field and the matching values in
	 *         the second
	 */
	 ...
	 *
	 * @param program
	 *            Command to be executed by the Runtime class
	 */
	 ...
	 * @param filters
	 *            Filters to be used with the dialog
	 * @return The file object on successful browsing, null otherwise
	 */
	public static File saveFileDialog(String startPath,
	 ...
	 * @return The file object on successful browsing, null otherwise
	 */
	public static File openFileDialog(String startPath,
			javax.swing.filechooser.FileFilter... filters)
	{
		final javax.swing.JFileChooser fc = new javax.swing.JFileChooser();
		fc.setAcceptAllFileFilterUsed(false);
	...
	/**
	 * Create a filter for JFileChooser. Extensions are given in a comma- delimited list
	 * with no preceding period. e.g. "txt,text" will create a filter for files with .txt
	 * or .text extension
	 *
	 * @param extensions
	 *            Comma-delimited list of extensions to filter for
	 * @param description
	 *            Description of the filter to be shown in the filechooser
	...
	/**
	 * Consume an argument and return a string array without the consumed argument
	 *
	 * @param args
	 *            Input string array
	 * @param index
	 *            Index of element to gobble
	...
	/**
	 * Takes a string representing a character enclosed by single quotes and checks for
	 * escaped characters. If escaped character is detected, the function will return the
	 * actual character code. It will only strip the single quotes otherwise. For example,
	 * 'A' will be returned as A as long, and '\n' will be returned as the newline
	 * character.
	 *
	 * @param str
	...
	/**
	 * NOT IMPLEMENTED YET Transform a string with embedded escapes to a character array
	 *
	 * @param str
	 *            String to transform
	 * @param escapes
	 ...
	 *
	 * @param parent
	 *            Parent component for dialog box
	 * @param str
	 *            String to display
	 * @param title
	 *            Title to display
	 ...
	 */
	public static boolean showYesNoDialog(java.awt.Component parent, String str,
			String title)
	{
		int ret;
	 ...
	 * @param t
	 *            Thread synchronization object. Useful for canceling the search before it
	 *            is completed via a thread and to display the current path being searched
	 * @return Absolute path of the file if found, null otherwise
	 */
	public static String findFileInDirectory(String dirStr, String fileStr,
			boolean recurse, ThreadSync t)
	{
		String ret = null;
	...
	/**
	 * Return a formatted hyperlink to be used by the IDE output to locate the source line
	 * (file:#, where # is the line number)
	 *
	 * @param fileName
	...
	/**
	 * A very hacky function to return a sorted list of &lt;String, Long&gt; by comparing
	 * the Long value
	 *
	 * @param map
	 *            HashMap that contains the key-value pairs
	...
	/**
	 * Parse a config file and return a hashmap with key-value pairs. The delimiter is
	 * "::"
	 *
	 * @param file
	 *            File to parse
	...
	public static HashMap<String, String> parseConfig(String file)
	{
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			if (tokens.length == 2)
			{
				Msg.D("config parse (" + file + "): " + tokens[0] + "::" + tokens[1], 3,
						null);
	 ...
	 *
	 * @param config
	 *            A HashMap with key-value config pairs
	 * @param file
	...
	public static void writeConfig(HashMap<String, String> config, String file)
	{
		...
		java.util.Iterator<java.util.Map.Entry<String, String>> i = config.entrySet()
				.iterator();
		while (i.hasNext())
		{
			e = i.next();
	...
	/**
	 * Get the command line arguments passed to the JVM when PLPTool was launched
	 *
	 ...
	 * something a little more brutual - this does the equivalent of "rm -r"
	 *
	 * http://stackoverflow.com/a/4026761 Paulitex
	 *
	 * @param path
	 *            Root File Path
	...
	/**
	 * Return the effective address of addresses in (base+offset) or (base-offset) format
	 *
	 * @param location
	 *            base+offset address in String
	 * @return resolved address in long, -1 if the address is failed to be resolved
	 */
	public static long resolveBaseOffset(PLPAsm asm, String location)
	{
		long ret = -1;
		StringTokenizer tokens = new StringTokenizer(location.replaceAll("\\s", ""),
				"+-", true);
		int tokenCount = tokens.countTokens();
		if (!tokens.hasMoreTokens())
		{
			return Msg.E("resolveBaseOffset: empty expression",
					Constants.PLP_NUMBER_ERROR, null);
		}
		String base = tokens.nextToken();
		long addr = asm.resolveAddress(base);
		ret = (addr == -1) ? parseNum(base) : addr;
		Msg.D("resolveBaseOffset: base=" + base + " asmresolved=" + format32Hex(ret)
				+ " tokens=" + tokenCount, 4, null);
		...
		{
			return Msg.E("resolveBaseOffset: invalid address expression",
					Constants.PLP_NUMBER_ERROR, null);
		}
		return ret;
	}
