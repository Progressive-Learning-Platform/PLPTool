...
public class InterruptController extends PLPSimBusModule
{
	...
	public int eval()
	{
		...
		if (gie && ((stat & mask) != 0) && ((SimCore) sim).IRQAck == 0)
		{
			...
			Msg.D("IRQ: " + String.format("%02x", sim.getIRQ()) + " stat: "
					+ String.format("%02x", stat) + " mask: "
					+ String.format("%02x", mask), 3, this);
	...
	{
		Msg.D("Writing " + ((Long) data) + " to " + String.format("0x%08x", addr), 1,
				this);
