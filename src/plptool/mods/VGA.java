...
/**
 * PLP VGA module. The VGA module takes the simulation bus as an argument as it draws a
 * contiguous region from memory, starting by the address pointed by the frame pointer.
 * gui_eval(x) is only executed when the control register is set to one.
 *
 * @author wira
 */
	...
	private int draw()
	{
		...
		for (int y_coord = 0; y_coord < 480; y_coord++)
		{
			for (int x_coord = 0; x_coord < 160; x_coord++)
			{
				...
				{
					/*
					 * -------------------- HACK ALERT ------------------------ This is a
					 * hack to prevent race condition, and may cause some artifact in VGA
					 * output. Unchecked read on the bus will return 'null' if the mapped
					 * module failed to return a value and will not generate a
					 * simulation-stopping error. This could happen when the module is
					 * being written to while we issue a read to the bus.
					 */
					Object dataObj = bus.uncheckedRead(addr);
					data = dataObj != null ? (Long) dataObj : 0;
					Msg.D("Initialized pixel at " + String.format("0x%08x", addr), 4,
							this);
				}
				...
				for (int i = 0; i < 4; i++)
				{
					...
					image[x_coord * 4 + (3 - i)][y_coord] = (red << 16) | (green << 8)
							| (blue);
				}
			}
		}
