...
/**
 * This class associates a module to its display frame, and allows users to dynamically
 * load / remove modules during runtime. Module developers need to add module information
 * and initialization here.
 *
...
{
	/**
	 * This is the data structure storing all the modules attached to this registry
	 */
	private ArrayList<PLPSimBusModule> modules;
	
	/**
	 * This list contains references to the GUI frames of attached PLPSimBusModule
	 * objects.
	 */
	...
	/**********************************************************************
	 * Number of modules registered. This constant needs to be incremented whenever new
	 * modules are added.
	 */
	...
	/**
	 * IORegistry constructor populates the module information array. Module developers
	 * will have to add their module's information here to be listed in the PLPTool I/O
	 * window.
	 */
	public IORegistry(ProjectDriver plp)
	{
		...
		mods[1][3] = "Simulated LED array. Will light up proper LEDs when "
				...
				+ "has one register and the register address must be " + "aligned.";
		mods[1][4] = true;
		mods[1][5] = true;
		...
		mods[4][3] = "This module always returns the board ID and frequency "
				+ "when the first and second registers are read, " + "respectively.";
		mods[4][4] = true;
		mods[4][5] = true;
	...
	/**
	 * Instantiate and attach a PLPSimBusModule-based module to this I/O registry and to
	 * the bus of the given simulation core. This method also associates a module with its
	 * internal frame, if GUI representation is implemented. Module developers will have
	 * to write their module's initialization here.
	 *
	 * @param index
	 *            Module type as registered in the constructor.
	 * @param addr
	 *            The starting address of the module's address space.
	 * @param size
	 *            The size of the module's address space.
	 * @return PLP_OK on completion, or PLP_SIM_INVALID_MODULE if invalid index is
	 *         specified.
	 */
		...
		// PLPSimMemModule is summoned
			...
			case 1:
				...
				if (plp.g())
				{
					...
					attachModuleFrameListeners((JFrame) moduleFrame, plp,
							Constants.PLP_TOOLFRAME_SIMLEDS);
				}
			...
			case 2:
				...
				if (plp.g())
				{
					...
					attachModuleFrameListeners((JFrame) moduleFrame, plp,
							Constants.PLP_TOOLFRAME_SIMSWITCHES);
				}
			...
			case 4:
				...
				if (plp.g())
				{
					...
					attachModuleFrameListeners((JFrame) moduleFrame, plp,
							Constants.PLP_TOOLFRAME_SIMPLPID);
				}
				break;
			...
			case 6:
				if (plp.g())
				{
					...
					attachModuleFrameListeners((JFrame) moduleFrame, plp,
							Constants.PLP_TOOLFRAME_SIMVGA);
				}
			...
			case 8:
				if (plp.g())
				{
					...
					attachModuleFrameListeners((JFrame) moduleFrame, plp,
							Constants.PLP_TOOLFRAME_SIMUART);
				}
			...
			case 9:
				...
				if (plp.g())
				{
					...
					attachModuleFrameListeners((JFrame) moduleFrame, plp,
							Constants.PLP_TOOLFRAME_SIMSEVENSEGMENTS);
				}
			...
			case 12:
				...
				if (plp.g())
				{
					...
					attachModuleFrameListeners((JFrame) moduleFrame, plp,
							Constants.PLP_TOOLFRAME_SIMGPIO);
				}
	...
	/**
	 * Instantiate and attach a dynamic PLPSimBusModule-based module to this I/O registry
	 * and to the bus of the given simulation core. PLPDynamicModuleFramework is called to
	 * create a new instance of the module.
	 *
	 * @param index
	 *            Index of the dynamic module CLASS
	 * @param addr
	 *            Starting address of the module's memory map
	 * @param size
	 *            Number of registers
	 * @param isWordAligned
	 *            Denote whether the module's address space is word aligned
	 * @param frame
	 *            Frame object associated with this module
	 * @return PLP_OK on completion, or PLP_DBUSMOD_INSTANTIATION_ERROR if the module
	 *         object failed to initialize
	 */
	public int attachDynamicModule(int index, long startAddr, long endAddr,
			boolean isWordAligned, Object frame)
	{
		...
		regSize.add(isWordAligned ? (endAddr - startAddr) / 4 + 1 : endAddr - startAddr
				+ 1);
		module.enable();
		positionInBus.add(plp.sim.bus.add(module));
	...
	/**
	 * @param index
	 *            Index of the module to be retrieved.
	...
	public Object getModuleFrame(int index)
	{
		if (index >= modules.size() || index < 0)
		{
			Msg.E("getModuleFrame: invalid index:" + index, Constants.PLP_GENERIC_ERROR,
					null);
			return null;
		}
	...
	public long getStartAddr(int index)
	{
		if (index >= modules.size() || index < 0)
		{
			Msg.E("getStartAddr: invalid index:" + index, Constants.PLP_GENERIC_ERROR,
					null);
			return -1;
		}
	...
	/**
	 * Removes all modules attached to this registry AND removes them from the simulation
	 * bus, effectively destroying all modules that were attached to the simulation.
	 *
	 * @param sim
	 *            Simulation core that is being run.
	 */
	 ...
	 *
	 * @param index
	 *            Index of the module to be removed.
	 * @param sim
	 *            Simulation core that is being run.
	 */
	...
	/**
	 * @return Module information in an object array. Used by the I/O window of the main
	 *         app to list available modules that are in this registry.
	 */
	...
	public int loadPreset(Preset preset)
	{
		...
		for (int i = 0; i < preset.size(); i++)
		{
			if (plp.g())
			{
				this.attachModuleToBus(preset.getType(i), preset.getAddress(i),
						preset.getSize(i));
	...
	public Preset createPreset()
	{
		...
		for (int i = 0; i < modules.size(); i++)
		{
			preset.addModuleDefinition(
					...
					(moduleFrames.get(i) instanceof JFrame) ? ((JFrame) moduleFrames
							.get(i)).isVisible() : false);
		}
	 ...
	 * @param plp
	 *            Current project driver instance
	 */
	public static void attachModuleFrameListeners(final JFrame x,
			final plptool.gui.ProjectDriver plp, final int menuDesignation)
	{
		x.addWindowListener(new java.awt.event.WindowListener() {
			...
			public void windowClosing(java.awt.event.WindowEvent evt)
			{
				...
				{
					Msg.D("IORegistry.attachModuleFrameListeners: menu designation "
							+ menuDesignation + " - no frame.", 3, this);
