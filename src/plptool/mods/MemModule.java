...
/**
 * PLP memory module. This module implements a wrapper read function that typecasts stored
 * objects to Long.
 *
 * @author wira
 */
...
{
	/**
	 * The constructor for the memory module takes the size of the memory in bytes if it's
	 * word aligned (such as main memory), or in words (such as register file).
	 *
	 * @param size
	 *            Size of memory module. In bytes if aligned, in words otherwise.
	 * @param wordAligned
	 *            Denotes whether this memory module is word aligned
	 */
	...
	/**
	 * The memory module should not have an evaluation procedure, this is here to satisfy
	 * implementation of the PLPSimBusModule superclass. Fancier memory modules can use
	 * this.
	 *
	 * @return PLP_OK
	 */
	...
	/**
	 * The memory module should not have an evaluation procedure, this is here to satisfy
	 * implementation of the PLPSimBusModule superclass.
	 *
	 * @return PLP_OK
	 */
	...
	public void printAll(long highlight)
	{
		...
		for (int i = 0; i < valueSet.length; i++)
		{
			...
			if (addr == highlight)
				Msg.pn(">>>");
			Msg.p(String.format("\t%08x\t%08x\t", addr, super.values.get(addr))
