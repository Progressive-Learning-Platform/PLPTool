...
public class PLPMIPSWebSim extends javax.swing.JApplet
{
	...
	private int step()
	{
		...
		{
			for (int j = 0; j < 4; j++)
				Msg.pn((i + 8 * j) + ((i + 8 * j < 10) ? "  " : " ")
						+ String.format("%08x", sc.regfile.read(i + 8 * j)) + "  ");
		...
		layout.setVerticalGroup(layout.createParallelGroup(
				...
				layout.createSequentialGroup()
						...
						.addGroup(
								layout.createParallelGroup(
										javax.swing.GroupLayout.Alignment.BASELINE)
										.addComponent(btnStep).addComponent(lblStatus)
										.addComponent(btnAssemble).addComponent(tglRun))
						.addContainerGap()));
	...
	{// GEN-FIRST:event_btnExecActionPerformed
		execCLI();
	}// GEN-LAST:event_btnExecActionPerformed
	
	private void txtCLIKeyPressed(java.awt.event.KeyEvent evt)
	{// GEN-FIRST:event_txtCLIKeyPressed
		if (evt.getKeyCode() == java.awt.event.KeyEvent.VK_ENTER)
			execCLI();
	}// GEN-LAST:event_txtCLIKeyPressed
	
	private void tglRunActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_tglRunActionPerformed
		toggleRun();
