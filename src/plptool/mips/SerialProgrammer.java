...
public class SerialProgrammer extends plptool.PLPSerialProgrammer
{
	...
	public int connect(String portName) throws Exception
	{
		...
		{
			return Msg.E(portName + " is not found.", Constants.PLP_GENERIC_ERROR, this);
		}
		...
		else
		{
			...
			{
				BAUDRATE = ((plptool.mips.Architecture) plp.getArch())
						.isUsingNexysBoard() ? 57600 : 115200;
	...
	public int programWithAsm() throws Exception
	{
		if (plp.asm.isAssembled() && out != null)
		{
			...
			Msg.D("Writing out first address " + String.format("0x%08x", addrTable[0]),
					2, this);
			buff[0] = (byte) 'a';
			...
			if (inData != 'f')
			{
				Msg.D("Acknowledgement byte: " + String.format("0x%x", inData), 2, this);
			...
			for (int i = 0; i < objCode.length && isProgramming(); i++)
			{
				...
				if (plp.g())
				{
					...
					if (!Config.prgProgramInChunks)
						...
						status = "Buffering " + progress + " of " + objCode.length
								+ " words";
				...
				if ((i < objCode.length - 1) && (addrTable[i + 1] != addrTable[i] + 4))
				{
					...
					Msg.D(String.format("Address jump: %08x to %08x", addrTable[i],
							addrTable[i + 1]), 3, this);
				...
				{
					if (!plp.g())
						Msg.I("Buffered " + progress + " of " + objCode.length + " words",
								this);
			...
			if (done)
			{
				...
				if (plp.g())
				{
					...
					plp.g_prg.getStatusField().setText(
							"Done. " + objCode.length + " words in " + timeSecs
									+ " seconds.");
				}
				Msg.I("done. " + objCode.length + " words in " + timeSecs + " seconds.",
						this);
	...
	private int sendChunk(byte[] chunk, int size, int i) throws IOException
	{
		...
		if (plp.g())
			plp.g_prg.getStatusField().setText(
					"Transmitting " + String.format("0x%08x", chunkStartAddr) + " to "
							+ String.format("0x%08x", addrTable[i]) + " (" + chunkIndex
							+ " bytes)");
		else
			Msg.I("Transmitting " + String.format("0x%08x", chunkStartAddr) + " to "
					+ String.format("0x%08x", addrTable[i]) + " (" + chunkIndex
					+ " bytes)", this);
	...
	public int sendPreamble(long preamble, InputStream in, OutputStream out)
			throws IOException
	{
		byte buff[] = new byte[5];
		byte inData = '\0';
