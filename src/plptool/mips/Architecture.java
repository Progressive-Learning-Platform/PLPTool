...
public class Architecture extends PLPArchitecture
{
		...
		if (plp.g())
		{
			...
			menuExportVerilogHex.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e)
				{
					...
					if (!plp.isAssembled())
					{
						Msg.E("Project needs to be assembled.",
								Constants.PLP_GENERIC_ERROR, null);
					}
					else
					{
						Msg.I("Generating Verilog hex-format...", null);
						hex = plptool.mips.Formatter.writeVerilogHex(plp.asm
								.getObjectCode());
						lines = hex.split("\n");
			...
			menuNexysBoard = new javax.swing.JCheckBoxMenuItem(
					"Use 57600 baud for programming (Nexys 2/3 board)");
	...
	/**
	 * Return a new instance of the assembler, using the asm source list array of the
	 * ProjectDriver as input
	 *
	...
	public void simulatorInitialization()
	{
		...
		if (plp.g())
		{
			...
			btnInt.setIcon(new javax.swing.ImageIcon(java.awt.Toolkit.getDefaultToolkit()
					.getImage(
							plp.g_dev.getClass().getResource(
									"resources/toolbar_exclamation.png"))));
			...
			btnInt.addActionListener(new java.awt.event.ActionListener() {
				...
				{
					((plptool.mods.Button) plp.sim.bus.getRefMod(1))
							.setPressedState(btnInt.isSelected());
				}
			});
		...
		if (plp.g())
		{
			...
			menuMemoryVisualizer.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e)
				{
					plptool.mips.visualizer.MemoryVisualization memvis = new plptool.mips.visualizer.MemoryVisualization(
							plp);
			...
			menuForgetMemoryVisualizer
					.addActionListener(new java.awt.event.ActionListener() {
						public void actionPerformed(java.awt.event.ActionEvent e)
						{
							((plptool.mips.SimCoreGUI) plp.g_sim)
									.disposeMemoryVisualizers();
							((plptool.mips.SimCoreGUI) plp.g_sim)
				...
				public void actionPerformed(java.awt.event.ActionEvent e)
				{
					if (menuBusMonitor.isSelected())
					{
						if (!busMonitorAttached)
						{
							...
							busMonitorFrame = new plptool.mods.BusMonitorFrame(
									(plptool.mods.BusMonitor) plp.sim.bus
											.getRefMod(busMonitorModulePosition),
									menuBusMonitor);
			...
			cpuVis = new plptool.mips.visualizer.CPUVisualization((SimCore) plp.sim,
					menuCpuVis);
			
			// Restore saved timing diagram from project attributes, if it exists
			plptimingdiagram.TimingDiagram savedTD = (plptimingdiagram.TimingDiagram) plp
					.getProjectAttribute("plpmips_timingdiagram");
			if (savedTD != null)
			{
				plptool.Msg.D(
						"Attempting to load timing diagram from project attributes.", 3,
						null);
				busMonitor = new plptool.mods.BusMonitor(plp.sim);
				...
				busMonitorFrame = new plptool.mods.BusMonitorFrame(
						(plptool.mods.BusMonitor) plp.sim.bus
								.getRefMod(busMonitorModulePosition),
						menuBusMonitor);
				Boolean b = (Boolean) plp
						.getProjectAttribute("plpmips_timingdiagram_framevisibility");
				if (b != null)
				{
	...
	public void simulatorStop()
	{
		...
		if (plp.g() && plp.g_dev != null)
		{
			...
			{
				plp.addProjectAttribute("plpmips_timingdiagram",
						busMonitor.getTimingDiagram());
				plp.addProjectAttribute("plpmips_timingdiagram_framevisibility",
						busMonitorFrame.isVisible());
	...
	public String saveArchSpecificSimStates()
	{
		...
		String ret = "";
		Object[][] attrSet = (Object[][]) plp
				.getProjectAttribute("plpmips_memory_visualizer");
		...
		plptimingdiagram.TimingDiagram tD = (plptimingdiagram.TimingDiagram) plp
				.getProjectAttribute("plpmips_timingdiagram");
		if (tD != null)
		{
	...
	public void restoreArchSpecificSimStates(String[] configStr)
	{
		if (configStr[0].equals("plpmips_memory_visualizer"))
		{
			...
			for (int j = 0; j < tokens.length; j++)
			{
				...
				plptool.Msg.D("plpmips_memory_visualizer load: " + tempTokens[0] + "-"
						+ tempTokens[1], 4, null);
	...
	public String getQuickReferenceString()
	{
		...
		str += "<p>Instructions: <a href=\"#r\">R-type</a> "
				+ "<a href=\"#i\">I-type</a> " + "<a href=\"#j\">J-type</a> " + "</p>";
		...
		data[0][0] = "<font color=blue><b>addu</b></font> $rd, $rs, $rt";
		data[0][1] = "rd = rs + rt";
		data[1][0] = "<font color=blue><b>subu</b></font> $rd, $rs, $rt";
		data[1][1] = "rd = rs - rt";
		data[2][0] = "<font color=blue><b>and</b></font>  $rd, $rs, $rt";
		data[2][1] = "rd = rs &amp; rt";
		data[3][0] = "<font color=blue><b>or</b></font>   $rd, $rs, $rt";
		data[3][1] = "rd = rs | rt";
		data[4][0] = "<font color=blue><b>nor</b></font>  $rd, $rs, $rt";
		data[4][1] = "rd = ~(rs | rt)";
		data[5][0] = "<font color=blue><b>mullo</b></font> $rd, $rs, $rt";
		data[5][1] = "rd = (rs * rt) &amp; 0xffffffff";
		data[6][0] = "<font color=blue><b>mulhi</b></font> $rd, $rs, $rt";
		data[6][1] = "rd = (rs * rt) &gt;&gt; 32";
		data[7][0] = "<font color=blue><b>slt</b></font>  $rd, $rs, $rt";
		data[7][1] = "rd = (rs &lt; rt) ? 1 : 0";
		data[8][0] = "<font color=blue><b>sltu</b></font> $rd, $rs, $rt";
		data[8][1] = "rd = (rs &lt; rt) ? 1 : 0";
		data[9][0] = "<font color=blue><b>sll</b></font>  $rd, $rt, shamt";
		data[9][1] = "rd = rt &lt;&lt; shamt";
		data[10][0] = "<font color=blue><b>srl</b></font>  $rd, $rt, shamt";
		data[10][1] = "rd = rt &gt;&gt; shamt";
		data[11][0] = "<font color=blue><b>sllv</b></font>  $rd, $rs, $rt";
		data[11][1] = "rd = rs &lt;&lt; rt";
		data[12][0] = "<font color=blue><b>srlv</b></font>  $rd, $rs, $rt";
		data[12][1] = "rd = rs &gt;&gt; rt";
		data[13][0] = "<font color=blue><b>jr</b></font> $rs";
		data[13][1] = "PC = rs";
		data[14][0] = "<font color=blue><b>jalr</b></font> $rd, $rs";
		data[14][1] = "rd = PC + 4; PC = rs";
		
		for (int i = 0; i < data.length; i++)
		{
			str += "<tr>";
			str += "<td><font face=\"monospaced\" size=\"12pt\">" + data[i][0]
					+ "</font></td><td>" + data[i][1] + "</td>";
			str += "</tr>";
		}
		...
		data[0][0] = "<font color=blue><b>addiu</b></font> $rt, $rs, imm";
		data[0][1] = "rt = rs + SignExtend(imm)";
		data[1][0] = "<font color=blue><b>andi</b></font>  $rt, $rs, imm";
		data[1][1] = "rt = rs & ZeroExtend(imm)";
		data[2][0] = "<font color=blue><b>ori</b></font>   $rt, $rs, imm";
		data[2][1] = "rt = rs | ZeroExtend(imm)";
		data[3][0] = "<font color=blue><b>slti</b></font>  $rt, $rs, imm";
		data[3][1] = "rt = (rs &lt; SignExtend(imm)) ? 1 : 0";
		data[4][0] = "<font color=blue><b>sltiu</b></font> $rt, $rs, imm";
		data[4][1] = "rt = (rs &lt; SignExtend(imm)) ? 1 : 0";
		data[5][0] = "<font color=blue><b>lui</b></font>   $rt, imm";
		data[5][1] = "rt = imm &lt;&lt; 16";
		data[6][0] = "<font color=blue><b>lw</b></font>    $rt, imm($rs)";
		data[6][1] = "rt = mem[SignExtend(imm) + rs]";
		data[7][0] = "<font color=blue><b>sw</b></font>    $rt, imm($rs)";
		data[7][1] = "mem[SignExtend(imm) + rs] = rt";
		data[8][0] = "<font color=blue><b>beq</b></font>   $rt, $rs, label";
		data[8][1] = "if (rt == rs) PC = PC + 4 + imm";
		data[9][0] = "<font color=blue><b>bne</b></font>   $rt, $rs, label";
		data[9][1] = "if (rt != rs) PC = PC + 4 + imm";
		...
		data[0][0] = "<font color=blue><b>j</b></font>     label";
		data[0][1] = "PC = label";
		data[1][0] = "<font color=blue><b>jal</b></font>   label";
		data[1][1] = "ra = PC + 4; PC = label";
		...
		data[0][0] = "<font color=blue><b>nop</b></font>";
		data[0][1] = "sll $0, $0, 0";
		data[1][0] = "<font color=blue><b>b</b></font>     label";
		data[1][1] = "beq $0, $0, label";
		data[2][0] = "<font color=blue><b>move</b></font>  $rd, $rs";
		data[2][1] = "or  $rd, $0, $rs";
		data[3][0] = "<font color=blue><b>li</b></font>    $rd, imm32";
		data[3][1] = "lui $rd, imm32 &gt;&gt; 16<br />";
		data[3][1] += "ori $rd, $rd, imm32 & 0xffff";
		data[4][0] = "<font color=blue><b>li</b></font>    $rd, label";
		data[4][1] = "lui $rd, label[31:16]<br />";
		data[4][1] += "ori $rd, $rd, label[15:0]";
		data[5][0] = "<font color=blue><b>push</b></font>  $rt";
		data[5][1] = "addiu $sp, $sp, -4<br />";
		data[5][1] += "sw $rt, 4($sp)";
		data[6][0] = "<font color=blue><b>pop</b></font>   $rt";
		data[6][1] = "lw $rt, 4($sp)<br />";
		data[6][1] += "addiu $sp, $sp, 4";
		data[7][0] = "<font color=blue><b>call</b></font>  label";
		data[7][1] = "Save $aX, $tX, and $ra to stack and call function";
		data[8][0] = "<font color=blue><b>return</b></font>";
		data[8][1] = "Restore $aX, $tX, and $ra from stack and return";
		data[9][0] = "<font color=blue><b>save</b></font>";
		data[9][1] = "Save all registers except for $zero to stack";
		data[10][0] = "<font color=blue><b>restore</b></font>";
		data[10][1] = "Restore all registers saved by 'save' in reverse order";
		data[11][0] = "<font color=blue><b>lwm</b></font> $rt, imm32/label";
		data[11][1] = "Load the value from a memory location into $rt";
		data[12][0] = "<font color=blue><b>swm</b></font> $rt, imm32/label";
		data[12][1] = "Store the value in $rt to a memory location";
		...
		data[0][0] = ".org <i>address</i>";
		data[0][1] = "Place subsequent statements starting from <i>address</i>";
		data[1][0] = "<i>label</i>:";
		data[1][1] = "Label current memory location as <i>label</i>";
		data[2][0] = ".word <i>value</i>";
		data[2][1] = "Write 32-bit <i>value</i> to the current address";
		data[3][0] = ".ascii \"<i>string</i>\"";
		data[3][1] = "Place <i>string</i> starting from the current address";
		data[4][0] = ".asciiz \"<i>string</i>\"";
		data[4][1] = "Place null-terminated <i>string</i> starting from the current address";
		data[5][0] = ".asciiw \"<i>string</i>\"";
		data[5][1] = "Place word-aligned <i>string</i> starting from the current address";
		data[6][0] = ".space <i>value</i>";
		data[6][1] = "Reserve <i>value</i> words starting from the current address";
		data[7][0] = ".equ <i>symbol</i> <i>value</i>";
		data[7][1] = "Add a symbol and its associated value to the symbol table (a constant)";
		...
		data[0][0] = "$0, $zero";
		data[0][1] = "The zero register";
		data[1][0] = "$1, $at";
		data[1][1] = "Assembler temporary";
		data[2][0] = "$2-$3, $v0-$v1";
		data[2][1] = "Return values";
		data[3][0] = "$4-$7, $a0-$a3";
		data[3][1] = "Function arguments";
		data[4][0] = "$8-$17, $t0-$t9";
		data[4][1] = "Temporaries";
		data[5][0] = "$18-$25, $s0-$s7";
		data[5][1] = "Saved temporaries";
		data[6][0] = "$26-$27, $i0-$i1";
		data[6][1] = "Interrupt temporaries";
		data[7][0] = "$28, $iv";
		data[7][1] = "Interrupt vector";
		data[8][0] = "$29, $sp";
		data[8][1] = "Stack pointer";
		data[9][0] = "$30, $ir";
		data[9][1] = "Interrupt return address";
		data[10][0] = "$31, $ra";
		data[10][1] = "Return address";
		...
		data[8][1] = "Timer";
		data[9][0] = "0xf0a00000";
		data[9][1] = "Seven Segments";
		data[10][0] = "0xf0700000";
		data[10][1] = "Interrupt Controller";
