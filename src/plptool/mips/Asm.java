...
/**
 * This class implements the modular PLP assembler object. The default constructor will
 * initialize required symbols for the assembler. The assembler class can handle multiple
 * assembly sources and cross-references.
 *
 * @author wira
 */
public class Asm extends plptool.PLPAsm
{
	...
	/**
	 * PLPAsm constructor. Read file described by strFilePath if strAsm is null, will
	 * attach strAsm to be assembled otherwise.
	 *
	 * @see preprocess(int)
	 * @see assemble()
	 * @param strAsm
	 *            String to assemble, null to load file instead
	 * @param strFilePath
	 *            Top level assembly source to attach
	 * @param intStartAddr
	...
	/**
	 * Define instruction->opcode mappings, instruction type mapping and register
	 * mappings.
	 */
	...
	/**
	 * Pre-process / perform 1st pass assembly on all assembly sources attached to this
	 * assembler. Resolve assembler directives, pseudo-ops and populate the symbol table.
	 *
	 * @return Returns 0 on completion, error code otherwise
	 * @param asmIndex
	 *            asm source index to preprocess
	 */
	public int preprocess(int index)
	{
		...
		try
		{
			...
			while (i < asmLines.length)
			{
				...
				if (commentSplit.length > 0)
				{
					...
					Msg.D(i + ": " + asmLines[i] + " tl: " + asmTokens.length, 5, this);
				...
				else if (asmTokens[0].equals(".include"))
				{
					...
					else
					{
						...
						for (int k = 0; k < sourceList.size(); k++)
						{
							...
							{
								Msg.W("preprocess("
										+ formatHyperLink(curActiveFile, i)
										+ "): "
										+ asmTokens[1]
										+ " is already in the project sources list. Ignoring.",
										this);
								found = true;
								conflict = true;
							}
						...
						if (!found)
						{
							...
							PLPAsmSource childAsm = new PLPAsmSource(null, asmTokens[1],
									asmIndex);
				...
				else if (asmTokens[0].equals(".org"))
				{
					...
					else
					{
						...
						if (entryPoint < 0)
							entryPoint = curAddr;
						Msg.D("curAddr is now " + String.format("0x%08x", curAddr), 5,
								this);
					}
				}
				...
				else if (asmTokens[0].equals(".text"))
				{
					...
					else if (curRegion != 1)
					{
						...
						{
							appendPreprocessedAsm(
									"ASM__ORG__ " + String.format("0x%08x", curTextAddr),
									i, true);
						}
				...
				else if (asmTokens[0].equals(".data"))
				{
					...
					else if (curRegion != 2)
					{
						...
						{
							appendPreprocessedAsm(
									"ASM__ORG__ " + String.format("0x%08x", curDataAddr),
									i, true);
						}
						...
						|| asmTokens[0].equals(".asciiw"))
				{
					...
					else
					{
						...
						Msg.D("l: " + tString.length + " :" + tString[tString.length - 1],
								6, this);
						...
						{
							if (tString[1].charAt(j) == '\\'
									&& j != tString[1].length() - 1)
							{
								...
								// Linefeed (0xA)
									case 'n':
										tString[1] = new StringBuffer(tString[1])
												.replace(j, j + 2, "\n").toString();
										break;
									...
									case 'r':
										tString[1] = new StringBuffer(tString[1])
												.replace(j, j + 2, "\r").toString();
										break;
									
									// Tab
									case 't':
										tString[1] = new StringBuffer(tString[1])
												.replace(j, j + 2, "\t").toString();
										break;
									...
									default:
										Msg.W("preprocess("
												...
												+ tString[1].charAt(j + 1), this);
								}
							}
						}
						
						// check if we need to append a null character for .asciiz ...
						int strLen = tString[1].length()
								+ ((asmTokens[0].equals(".asciiz")) ? 1 : 0);
						...
						for (j = 0; j < strLen; j++)
						{
							...
							if (!wordAligned)
								appendPreprocessedAsm(
										String.format("%02x", (int) tString[1].charAt(j)),
										i, false);
							else
							{
								appendPreprocessedAsm(
										String.format("%08x", (int) tString[1].charAt(j)),
										i, true);
								regionMap.add(curRegion);
								curAddr += 4;
							}
				...
				else if (asmLines[i - 1].equals("")
						|| (asmTokens[0].length() > 0 && asmTokens[0].charAt(0) == '#'))
				{
					appendPreprocessedAsm("ASM__SKIP__", i, true);
					directiveOffset++;
				}
				...
				// line
				else if (asmTokens[0].length() > 1
						&& asmTokens[0].charAt(asmTokens[0].length() - 1) == ':')
				{
				...
				else if (asmTokens[0].equals("move"))
				{
					...
					{
						appendPreprocessedAsm("or " + asmTokens[1] + ",$0,"
								+ asmTokens[2], i, true);
						regionMap.add(curRegion);
						curAddr += 4;
					}
				...
				else if (asmTokens[0].equals("li"))
				{
					...
					{
						appendPreprocessedAsm("lui " + asmTokens[1] + ",$_hi:"
								+ asmTokens[2], i, true);
						appendPreprocessedAsm("ori " + asmTokens[1] + "," + asmTokens[1]
								+ ",$_lo:" + asmTokens[2], i, true);
						regionMap.add(curRegion);
						regionMap.add(curRegion);
						curAddr += 8;
					}
		...
		if (error > 0)
			Msg.E("preprocess: " + error + " error(s).",
					Constants.PLP_ASM_PREPROCESS_FAILED, this);
		
		Msg.D("preprocess(" + sourceList.get(index).getAsmFilePath()
				+ "): First pass completed.", 2, this);
	...
	/**
	 * Assemble all assembly sources attached to this assembler and generate object codes.
	 * Populates objectCodes[] and addrTable[] and sets the boolean assembled on
	...
	public int assemble()
	{
			...
			while (i < asmLines.length)
			{
				...
				// 3-op R-type (includes multiply, case 8)
					...
					case 8:
						...
						{
							error++;
							errMsg = "assemble(" + formatHyperLink(filePath, lineNumber)
									+ "): Invalid register(s)";
					...
					case 3:
						...
						else if (!symTable.containsKey(asmTokens[3]))
						{
							error++;
							errMsg = "assemble(" + formatHyperLink(filePath, lineNumber)
									+ "): Invalid branch target \"" + asmTokens[3] + "\"";
					...
					case 7:
						...
						else if (!symTable.containsKey(asmTokens[1]))
						{
							error++;
							errMsg = "assemble(" + formatHyperLink(filePath, lineNumber)
									+ "): Invalid jump target \"" + asmTokens[1] + "\"";
							Msg.E(errMsg, Constants.PLP_ASM_INVALID_BRANCH_TARGET, this);
							addBuildError(fileIndex, lineNumber, errMsg);
						}
						else
		...
		if (error > 0)
			Msg.E("assemble: " + error + " error(s).", Constants.PLP_ASM_ASSEMBLE_FAILED,
					this);
		else
		{
			Msg.I("Total statically allocated memory: <b>"
					+ (objectCode.length + bytesSpace / 4) + "</b> words", this);
			Msg.I("Object code and initialized variables: <b>" + objectCode.length
					+ "</b> words", this);
			assembled = true;
		}
	...
	/**
	 * Return whether the object code entry in the given index is an instruction (zero) or
	 * not (non-zero)
	 *
	 * @return Returns the entry type ID
	 * @param index
	 *            Index of the entry in the object code array
	 */
	 ...
	 * @param instrOpCode
	 *            the opcode of the instruction in (byte)
	 */
	...
	/**
	 * Lookup instruction type of the provided opcode as defined by the PLPAsm class.
	 *
	 * @return Returns instruction type
	 * @param instrOpCode
	 *            Instruction opcode in String
	...
	/**
	 * Take in a string and attempts to parse it as a 16 bit number. This 16-bit number is
	 * stored in int primitive (32-bit data type) to preserve all 16-bits of data since
	 * Java doesn't have unsigned data types. Higher 2-bytes of the number are masked.
	 *
	 * @return Returns 16-bit number in long with higher 2-bytes masked , returns
	 *         PLP_NUMBER_ERROR if parseInt failed.
	 * @param Number
	 *            to be sanitized in String
	...
	/**
	 * Take in a string and attempts to parse it as a 32 bit number. This 32-bit number is
	 * stored in long primitive (64-bit data type) to preserve all 32-bits of data since
	 * Java doesn't have unsigned data types. Higher 4-bytes of the number are masked.
	 *
	 * @return Returns 32-bit number in long with higher 4-bytes masked , returns
	 *         PLP_NUMBER_ERROR if parseLong failed.
	 * @param Number
	 *            to be sanitized in String
	 ...
	 *            assertion
	 * @param lineNum
	 *            line number in asm source where this function is called
	 */
	private boolean checkNumberOfOperands(Object iObj[], int length, int asmLineNum)
	{
		if (iObj.length != length)
		{
			...
			String msg = "assemble("
					+ formatHyperLink(sourceList.get(fileIndex).getAsmFilePath(),
							lineNumber) + "): Invalid number of operands";
	 ...
	 * @param lineNumber
	 *            Line number that causes the error
	 * @param message
	...
	public int preSimulationCheck()
	{
		if (getObjectCode().length == 0)
			return Msg.E("simulate: Empty program.", Constants.PLP_SIM_EMPTY_PROGRAM,
					this);
