...
/**
 * mips.SimCore is the PLP CPU Architecture Simulator. This class contains core
 * functionality of the simulator: the CPU itself, memory elements, pipeline registers,
 * and I/O devices. Users can interact with the simulation core by using typical
 * procedures such as stepping and I/O. It's up to the command line interface or the GUI
 * to present these features to the user.
 *
 * @author wira
 */
public class SimCore extends PLPSimCore
{
	 ...
	 *
	 * @param RAMsize
	 *            the size of main memory attached to this core
	 * @param startAddr
	 *            the starting address of the main memory
	 * @param asm
	 *            assembler object passed on to this simulator
	 * @see plptool.PLPAsm
	 */
	...
	/**
	 * Reset the core: clears RAM and zeroes out register file, reloads the program to
	 * memory, resets program counter, flushes the pipeline, clears flags, and resets
	 * statistics.
	 *
	...
	/**
	 * Part of the PLPSimCore specification, this function implements a program loading
	 * routine for the simulation core.
	 *
	 * @param x
	 *            assembler object containing the program code
	...
	public int step()
	{
		...
		else
		{
			...
			if (ret != 0)
			{
				...
				if (Config.simDumpTraceOnFailedEvaluation)
					this.registersDump();
			}
			 ...
			 * STALL ROUTINES
			 * 
			 * By default, the CPU here will just get the next instruction as determined
			 * by the current value of PC. This is done by updating the input side of the
			 * IF/ID pipeline register (id_stage.i_* values) by calling the fetch()
			 * function.
			 * 
			 * There are three conditions where this is not true:
			 * 
			 * 1. IF stall due to jumps (id_stage.i_* values will take a bubble and no new
			 * instruction will be fetched, i.e. fetch() function will not be called in
			 * this case).
			 * 
			 * 2. Stall on EX stage due to load-use hazard. Insert bubble for EX in the
			 * next cycle by making sure ex_stage.i_* values will not change the CPU state
			 * (no write to memory and register, no branch and jump). Then, the IF/ID
			 * pipeline is turned off by making id_stage.hot = false. This will keep the
			 * instruction being decoded to stay in that stage. fetch() will be called to
			 * fill the IF stage or the id_stage.i_* values. Finally we rewrite PC so we
			 * don't skip an instruction since PC is already clocked due to the procedural
			 * nature of our simulation engine. In a real machine, the PC would have held
			 * its value.
			 * 
			 * 3. An interrupt service is requested. The interrupt service is a 3-step
			 * process. First, when a request is detected in the end of a clock cycle, the
			 * CPU checks whether a jump or a branch has been executed in the EX stage. If
			 * yes, the CPU will wait another cycle before servicing (the next instruction
			 * is fetched in the IF stage as usual). If a jump or branch is not in the EX
			 * stage, the CPU will go ahead and flush the IF, ID, and EX stages for the
			 * next cycle.and record the address of the instruction that was in the EX
			 * stage. In the next cycle, a jalr $ir, $iv instruction is injected in the IF
			 * stage (id_stage.i_*) with the return value being the recorded address minus
			 * 4 to offset the plus 4 of the PC logic. During the third cycle, the CPU
			 * injects a bubble for the jump and resumes normal operation in the ISR
			 * space.
			 */
			...
			else if (int_state == 3)
			{
				...
				if (diff == 8)
				{
					...
					int_state--;
					IRQAck = 1;
					Msg.D("IRQ service started, int_inject = 2, irq_ret = "
							+ String.format("0x%02x", irq_ret), 3, this);
	...
	private int fetch()
	{
		...
		if (!bus.isMapped(addr) || addr < 0)
		{
			if (Config.simDumpTraceOnFailedEvaluation)
				this.registersDump();
			return Msg.E("fetch(): PC points to unmapped address. Halt." + " pc="
		...
		if (!bus.isInstr(addr) && !Config.simAllowExecutionOfArbitraryMem)
			return Msg.E("fetch(): Attempted to fetch non-executable memory: " + "pc="
		...
		Msg.D("fetch(): PC input side: " + String.format("0x%08x", pc.input())
				+ " - PC output side: " + String.format("0x%08x", pc.eval()), 5, this);
	...
	/**
	 * Non-cycle accurate stepping. One instruction is fetched and executed every cycle.
	 * Branch delay slots are simulated.
	 *
	 * @return Returns 0 on successful completion. Error code otherwise.
	 */
	public int stepFunctional()
	{
		...
		{
			Msg.D("INT REQ - ret_addr(pcplus4-4) = "
					+ plptool.PLPToolbox.format32Hex(pcplus4 - 4), 4, this);
			sim_flags |= PLP_SIM_IRQ;
			// rewrite instruction to jalr $iv, $ir
			instr = 0x0380f009L;
			id_stage.i_instrAddr = 0;
			pcplus4 -= 8; // replay the discarded instruction after return from IRQ (jalr
							// adds 4)
			int_state = 1;
		}
		...
		{
			Msg.D("DELAY SLOT - branching to = "
					+ plptool.PLPToolbox.format32Hex(branch_destination), 4, this);
	...
	{
		Msg.p("if:   "
				+ ((id_stage.i_instrAddr == -1 || id_stage.i_bubble) ? "--------"
						: String.format("%08x", id_stage.i_instrAddr)) + " instr: "
				+ String.format("%08x", id_stage.i_instruction) + " : "
				+ MIPSInstr.format(id_stage.i_instruction) + "\ni_pc: "
				+ String.format("%08x", pc.input()));
	}
	 ...
	 *
	 * @param highlight
	 *            Memory location to highlight, probably the PC value
	 */
	public void printProgram(int memoryPosition, long highlight)
	{
		...
		for (int i = 0; i < values.length; i++)
		{
			...
			if ((Boolean) values[i][2])
				Msg.p(String.format("\t%08x\t%08x\t", (Long) values[i][0],
						(Long) values[i][1]) + MIPSInstr.format((Long) values[i][1]));
		}
		Msg.P();
	}
	...
	public void registersDump()
	{
		...
		if (wb_i > -1)
		{
			...
			Msg.M(" wb:  " + asms.get(wb_index).getAsmFilePath() + ":" + wb_line + " "
					+ asms.get(wb_index).getAsmLine(wb_line));
		}
		else if (wb_i < 0)
			Msg.M(" wb:  -- bubble --");
		...
		if (mem_i > -1)
		{
			...
			Msg.M(" mem: " + asms.get(mem_index).getAsmFilePath() + ":" + mem_line + " "
					+ asms.get(mem_index).getAsmLine(mem_line));
		}
		else if (mem_i < 0)
			Msg.M(" mem: -- bubble --");
		...
		if (ex_i > -1)
		{
			...
			Msg.M(" ex:  " + asms.get(ex_index).getAsmFilePath() + ":" + ex_line + " "
					+ asms.get(ex_index).getAsmLine(ex_line));
		}
		else if (ex_i < 0)
			Msg.M(" ex:  -- bubble --");
		...
		if (id_i > -1)
		{
			...
			Msg.M(" id:  " + asms.get(id_index).getAsmFilePath() + ":" + id_line + " "
					+ asms.get(id_index).getAsmLine(id_line));
		}
		else if (id_i < 0)
			Msg.M(" id:  -- bubble --");
		...
		if (pc_i > -1)
		{
			...
			Msg.M(" if:  " + asms.get(pc_index).getAsmFilePath() + ":" + pc_line + " "
					+ asms.get(pc_index).getAsmLine(pc_line));
		}
		else if (pc_i < 0)
			Msg.M(" if:  -- bubble --");
	}
	...
	/**
	 * This is the instruction decode (ID) / register file (RF) stage of the PLP CPU core.
	 * The execute module and the register file are attached to this class.
	 */
	public class id
	{
		...
		{
			Msg.p("ID vars");
			Msg.p("\tinstruction:             " + String.format("%08x", instruction)
					+ " " + MIPSInstr.format(instruction));
			Msg.p("\tinstrAddr:               "
					+ ((instrAddr == -1) ? "--------" : String.format("%08x", instrAddr)));
		...
		{
			Msg.p("ID next vars");
			Msg.p("\ti_instruction:           " + String.format("%08x", i_instruction)
					+ " " + MIPSInstr.format(i_instruction));
			Msg.p("\ti_instrAddr:             "
					+ ((i_instrAddr == -1) ? "--------" : String.format("%08x",
							i_instrAddr)));
		...
		{
			return "id:   "
					+ ((instrAddr == -1 || bubble) ? "--------" : String.format("%08x",
							instrAddr)) + " instr: " + String.format("%08x", instruction)
					+ " : " + MIPSInstr.format(instruction);
		}
		...
		private int eval()
		{
			try
			{
				...
				if (mem_stage.hot && mem_ex_lw)
				{
					if (ex_rt == addr_rt && addr_rt != 0 && ex_stage.fwd_ctl_memread == 1
							&& (MIPSInstr.opcode(instruction) != 0x2B))
					{ // lw->sw is mem->mem if it's rt
						ex_stall = true;
				...
				// rt
				ex_reg.i_data_rt = (addr_rt == 0) ? 0 : (Long) regfile.read(addr_rt);
				
				// rs
				ex_reg.i_data_rs = (addr_rs == 0) ? 0 : (Long) regfile.read(addr_rs);
				...
				if (opcode != 0x0C && opcode != 0x0D)
					...
					ex_reg.i_data_imm_signExtended = (short) imm_field
							& ((long) 0xfffffff << 4 | 0xf);
				else
			...
			{
				return Msg.E("I screwed up: " + e, Constants.PLP_SIM_EVALUATION_FAILED,
						this);
			}
		}
	...
	/**
	 * This is the execute (EX) stage of the PLP CPU pipeline. The memory and ALU modules
	...
	public class ex
	{
		...
		public void printvars()
		{
			String rt_forwarded = (sim_flags & (PLP_SIM_FWD_EX_EX_RT | PLP_SIM_FWD_MEM_EX_RT)) == 0 ? ""
					: " (forwarded)";
			String rs_forwarded = (sim_flags & (PLP_SIM_FWD_EX_EX_RS | PLP_SIM_FWD_MEM_EX_RS)) == 0 ? ""
					: " (forwarded)";
			...
			Msg.p("\tdata_imm_signExtended:   "
					+ String.format("%08x", data_imm_signExtended));
			Msg.p("\tdata_rs:                 " + String.format("%08x", data_rs));
			Msg.p("\tdata_rt:                 " + String.format("%08x", data_rt));
			Msg.p("\tdata_x (ALU0)*:          " + String.format("%08x", data_x)
					+ rs_forwarded);
			Msg.p("\tdata_eff_y*:             " + String.format("%08x", data_eff_y)
					+ rt_forwarded);
		...
		public void printnextvars()
		{
			...
			Msg.p("\ti_fwd_ctl_linkaddr:      "
					+ String.format("%08x", i_fwd_ctl_linkaddr));
			...
			Msg.p("\ti_ctl_branchtarget:      "
					+ String.format("%08x", i_ctl_branchtarget));
			Msg.p("\ti_ctl_jump:              " + i_ctl_jump);
			Msg.p("\ti_ctl_branch:            " + i_ctl_branch);
			
			Msg.p("\ti_data_imm_signExtended: "
					+ String.format("%08x", i_data_imm_signExtended));
		...
		private int eval()
		{
			try
			{
				...
				ctl_forwardX = (ex_ex && mem_ctl_regwrite
						&& mem_stage.fwd_ctl_dest_reg_addr == ex_rs && ex_rs != 0) ? 1
						: (mem_ex && wb_ctl_regwrite
								&& wb_stage.ctl_dest_reg_addr == ex_rs && ex_rs != 0) ? 2
								: 0;
				...
				ctl_forwardY = (ex_ex && mem_ctl_regwrite
						&& mem_stage.fwd_ctl_dest_reg_addr == ex_rt && ex_rt != 0) ? 1
						: (mem_ex && wb_ctl_regwrite
								&& wb_stage.ctl_dest_reg_addr == ex_rt && ex_rt != 0) ? 2
								: 0;
				...
				internal_alu_out = exAlu.eval(data_x, data_y, ctl_aluOp)
						& (((long) 0xfffffff << 4) | 0xf);
				...
				mem_reg.i_fwd_ctl_dest_reg_addr = (ctl_regDst == 1) ? ctl_rd_addr
						: ctl_rt_addr;
				...
				ctl_jumptarget = (jtype == 7) ? (instrAddr & 0xF0000000)
						| (MIPSInstr.jaddr(instruction) << 2) : data_rs;
	...
	/**
	 * This is the memory (MEM) stage of the PLP CPU pipeline. Writeback module is
	 * attached to this class. The bus module is also attached to this class, and it is
	 * the only place where the PLP CPU simulation core interacts with the outside world
	 * aside from fetching instructions in IF stage.
	 */
	public class mem
	{
		public int count = 0;
		...
		public void printvars()
		{
			String writedata_forwarded = (sim_flags & (PLP_SIM_FWD_MEM_MEM)) == 0 ? ""
					: " (forwarded)";
			...
			Msg.p("\tfwd_ctl_linkaddr:        " + String.format("%08x", fwd_ctl_linkaddr));
			Msg.p("\tfwd_ctl_jal:             " + fwd_ctl_jal);
			Msg.p("\tfwd_data_alu_result:     "
					+ String.format("%08x", fwd_data_alu_result));
			
			Msg.p("\tctl_memwrite:            " + ctl_memwrite);
			Msg.p("\tctl_memread:             " + ctl_memread);
			Msg.p("\tctl_fwd_mem_mem*:        " + ctl_fwd_mem_mem);
			
			Msg.p("\tdata_memwritedata:       "
					+ String.format("%08x", data_memwritedata));
			Msg.p("\tdata_mem_load*:          " + String.format("%08x", data_mem_load));
			Msg.p("\tdata_mem_store*:         " + String.format("%08x", data_mem_store)
					+ writedata_forwarded);
			Msg.P();
		}
		...
		public void printnextvars()
		{
			...
			Msg.p("\ti_fwd_ctl_linkaddr:      "
					+ String.format("%08x", i_fwd_ctl_linkaddr));
			Msg.p("\ti_fwd_ctl_jal:           " + i_fwd_ctl_jal);
			Msg.p("\ti_fwd_data_alu_result:   "
					+ String.format("%08x", i_fwd_data_alu_result));
			
			Msg.p("\ti_ctl_memwrite:          " + i_ctl_memwrite);
			Msg.p("\ti_ctl_memread:           " + i_ctl_memread);
			
			Msg.p("\ti_data_memwritedata:     "
					+ String.format("%08x", i_data_memwritedata));
			Msg.P();
		}
		...
		private int eval()
		{
			try
			{
				...
				data_mem_store = (ctl_fwd_mem_mem == 1) ? wb_stage.data_regwrite
						: data_memwritedata;
				...
				data_mem_load = (ctl_memread == 1) ? (Long) bus.read(fwd_data_alu_result)
						: 0;
	...
	/**
	 * This is the writeback (WB) and final stage of the PLP CPU pipeline. Instructions
	 * are retired and register file is written to in this stage.
	 */
	public class wb
	{
		public int count = 0;
		...
		public void printnextvars()
		{
			...
			Msg.p("\ti_data_memreaddata:      "
					+ String.format("%08x", i_data_memreaddata));
			Msg.p("\ti_data_alu_result:       "
					+ String.format("%08x", i_data_alu_result));
			Msg.P();
		}
		...
		private int eval()
		{
			try
			{
				...
				long internal_2x1 = (ctl_jal == 0) ? data_alu_result : ctl_linkaddr;
