...
public class SimCLI
{
	...
	public static void simCLCommand(String cmd, plptool.gui.ProjectDriver plp)
	{
		...
		else if (input.equals("i"))
		{
			...
			cmd_listmods();
		else if (input.equals("listdmods"))
			cmd_listdmods();
			...
			cmd_addmod();
		else if (tokens[0].equals("adddmod"))
			cmd_adddmod();
	...
	public static void cmd_s()
	{
		if (core.stepW() != Constants.PLP_OK)
			Msg.E("Simulation is stale. Please reset.", Constants.PLP_SIM_STALE, null);
		else if (!silent)
		{
			Msg.P("");
	...
	public static void cmd_s_arg()
	{
		...
		else
		{
			...
			{
				if (core.stepW() != Constants.PLP_OK)
					Msg.E("Simulation is stale. Please reset.", Constants.PLP_SIM_STALE,
							null);
				else if (!silent)
				{
					Msg.P("");
				...
				if (core.breakpoints.isBreakpoint(core.visibleAddr))
					Msg.P("--- stopping at breakpoint: "
							+ PLPToolbox.format32Hex(core.visibleAddr));
			}
			if (steps > Constants.PLP_LONG_SIM)
			{
		...
		else
		{
			for (int i = 0; i < ioReg.getNumOfModsAttached(); i++)
			{
				if (ioReg.getType(i) == 0)
				{
					...
					((plptool.mods.MemModule) ioReg.getModule(i)).print(PLPToolbox
							.parseNum(tokens[1]));
				}
			}
		}
	...
	public static void cmd_preg()
	{
		...
		for (int j = 0; j < 32; j++)
		{
			...
			Msg.p(j + "\t" + String.format("%08x", data) + "\t"
	...
	public static void cmd_rbus()
	{
		...
		else
		{
			...
			if (addr > -1)
			{
				...
				{
					long value = (Long) ret;
					Msg.P(String.format("0x%08x=", addr) + String.format("0x%08x", value));
				}
			}
			else
				Msg.E("Invalid address.", Constants.PLP_NUMBER_ERROR, null);
	...
	public static void cmd_addmod()
	{
		...
		else
		{
			ioReg.attachModuleToBus((int) PLPToolbox.parseNum(tokens[1]),
					PLPToolbox.parseNum(tokens[2]), PLPToolbox.parseNum(tokens[3]));
		}
	}
	...
	public static void cmd_attachedmods()
	{
		...
		{
			Msg.P(i + ": " + ((plptool.PLPSimBusModule) mods[i]).introduce()
	...
	public static void cmd_asm()
	{
		...
		else
		{
			...
			if (inlineAsm.isAssembled() && base > -1)
			{
				...
				{
					addr = base + 4 * j;
					core.bus.write(addr, inlineAsm.getObjectCode()[j],
							(inlineAsm.isInstruction(j) == 0) ? true : false);
	...
	public static void cmd_bp()
	{
		...
		else if (tokens[1].equals("set") && tokens.length == 3)
		{
			...
			long addr = (addrLabel == -1) ? PLPToolbox.parseNumSilent(tokens[2])
					: addrLabel;
			if (addr > -1)
			{
				core.breakpoints.add(addr, asm.getFileIndex(addr), asm.getLineNum(addr));
				Msg.P("Breakpoint set at " + PLPToolbox.format32Hex(addr)
						+ (addrLabel != -1 ? " (" + tokens[2] + ")" : ""));
			}
			else
		...
		else if (tokens[1].equals("list"))
		{
			...
			for (int i = 0; i < core.breakpoints.size(); i++)
			{
				...
				Msg.P(i + "\t" + PLPToolbox.format32Hex(addr)
						+ (label != null ? " " + label : ""));
			}
		}
