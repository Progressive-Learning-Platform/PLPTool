...
public class MemoryVisualization extends javax.swing.JFrame
{
	...
	public MemoryVisualization(ProjectDriver plp)
	{
		...
		this.setIconImage(java.awt.Toolkit.getDefaultToolkit().getImage(
				this.getClass().getResource("resources/toolbar_memvis.png")));
	}
	...
	private void initComponents()
	{
		...
		org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application
				...
				.getResourceMap(MemoryVisualization.class);
	...
	private void btnVisualizeActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_btnVisualizeActionPerformed
		try
		{
	...
	private void txtStartAddrKeyPressed(java.awt.event.KeyEvent evt)
	{// GEN-FIRST:event_txtStartAddrKeyPressed
		if (evt.getKeyCode() == java.awt.event.KeyEvent.VK_ENTER)
			btnVisualizeActionPerformed(null);
	}// GEN-LAST:event_txtStartAddrKeyPressed
	
	private void txtOffsetKeyPressed(java.awt.event.KeyEvent evt)
	{// GEN-FIRST:event_txtOffsetKeyPressed
		if (evt.getKeyCode() == java.awt.event.KeyEvent.VK_ENTER)
			btnVisualizeActionPerformed(null);
	...
	class DrawPanel extends JPanel
	{
		...
		{
			g.setFont(new Font(plptool.Config.devFont, Font.BOLD,
			...
			g.drawString("Contents", W - 10 - addrStrOffset - rightOffset,
					4 + fm.getHeight());
			g.drawString("Address", W - 30 - 2 * addrStrOffset - rightOffset,
					4 + fm.getHeight());
			g.drawString(
					"$sp",
					W - 40 - 2 * addrStrOffset - 30 - rightOffset - fm.stringWidth("$sp"),
					4 + fm.getHeight());
			...
			for (int i = 0; i < locs; i++)
			{
				...
				if (spVal == null)
					spVal = oldSpVal;
				if (spVal >= 0 && spVal >= startAddr + addrOffset * i
						&& spVal < startAddr + addrOffset * i + addrOffset)
				{
					g.setColor(Color.red);
					int xPoints[] = { W - 40 - 2 * addrStrOffset - 30 - rightOffset,
							W - 40 - 2 * addrStrOffset - 30 - rightOffset,
							W - 40 - 2 * addrStrOffset - 10 - rightOffset };
					int yPoints[] = { topOffset + i * rowH + rowH / 2 - 5,
							topOffset + i * rowH + rowH / 2 + 5,
							topOffset + i * rowH + rowH / 2 };
				...
				g.fillRect(W - 20 - addrStrOffset - rightOffset, topOffset + i * rowH,
						20 + addrStrOffset, rowH);
					...
					if (yScaleFactor <= 5 && isMapped)
					{
						...
						g.drawString(
								String.format("0x%08x",
										plp.sim.bus.read(startAddr + addrOffset * i)), W
										- 10 - addrStrOffset - rightOffset, topOffset + i
										* rowH + stringYOffset);
					}
					g.setColor(plp.sim.visibleAddr == startAddr + addrOffset * i ? Color.red
							: Color.black);
					g.drawString(String.format("0x%08x", startAddr + addrOffset * i), W
							- 10 - 2 * addrStrOffset - 20 - rightOffset, topOffset + i
							* rowH + stringYOffset);
				}
			}
			...
			g.setColor(Color.red);
			if (yScaleFactor >= 23)
				g.drawString((int) (Math.pow(2, (yScaleFactor - 23))) + " Mbytes / div",
						5, topOffset + fm.getDescent() + 10);
			else if (yScaleFactor >= 13)
				g.drawString((int) (Math.pow(2, (yScaleFactor - 13))) + " Kbytes / div",
						5, topOffset + fm.getDescent() + 10);
			else
				g.drawString(
						(int) (Math.pow(2, (yScaleFactor - 5))) * 4 + " bytes / div", 5,
						topOffset + fm.getDescent() + 10);
