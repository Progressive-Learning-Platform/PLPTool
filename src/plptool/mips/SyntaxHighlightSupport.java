...
public class SyntaxHighlightSupport implements PLPSyntaxHighlightSupport
{
	...
	public static Color[] syntaxColors = { new Color(0, 0, 255), new Color(0, 0, 255),
			new Color(0, 0, 255), new Color(0, 0, 255), new Color(0, 0, 255),
			new Color(127, 127, 127), new Color(255, 0, 0), new Color(255, 153, 0),
			new Color(0, 0, 0), new Color(0, 153, 0), new Color(204, 0, 102) };
	public static boolean[] syntaxBold = { false, false, false, false, false, false,
			false, false, true, false, false };
	public static boolean[] syntaxItalic = { false, false, false, false, false, false,
			false, false, false, false, false };
	
	public void syntaxHighlightTextAction(ProjectDriver plp, String text, int position)
	{
		if (styles == null)
			newStyle();
		...
		while (currentposition < text.length())
		{
			...
			while (currentposition < text.length()
					&& (text.charAt(currentposition) == '\t'
							|| text.charAt(currentposition) == ' ' || text
			...
			doc.setCharacterAttributes(startposition + position, currentposition
					- startposition, styles[texttype], false);
			currentposition++;
		}
	}
	...
	private boolean isimmediate(String num)
	{
		...
		return (matcher0.matches() || matcher1.matches() || matcher2.matches() || matcher3
				.matches());
	}
