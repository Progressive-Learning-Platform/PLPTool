...
/**
 * This class implements the simulation core bus / front-side bus. Simulator users can
 * attach I/O module classes that extend the PLPSimBusModule abstract class. This object
 * is ideally used by the simulation core during memory access. For example, the PLPTool
 * MIPS simulation core attaches its memory module to the bus, and performs bus write and
 * read during the MEM stage of the pipeline. All modules attached to the bus can be
 * accessed through this class' methods, or directly using the getModRef function.
 *
 * @see PLPSimCore
...
public class PLPSimBus
{
	...
	/**
	 * The default constructor for the class will start with a bus with nothing attached
	 * to it. This may be preferable for CPUs that do not use this bus to write to main
	 * memory.
	 *
	...
	/**
	 * This constructor will attach the provided memory module to the bus. Can be used
	 * with simulator cores with main memory attached to the bus.
	 *
	 * @param PLPSimCore
	 *            The simulator core
	 * @see PLPSimBus()
	 ...
	 *
	 * @param module
	 *            PLPSimBusModule-based class to be added
	...
	public int add(PLPSimBusModule module)
	{
		...
		if (ret)
		{
			...
			return Msg.E("Failed to attach module " + module, Constants.PLP_ERROR_RETURN,
					this);
	}
	...
	/**
	 * Issue a read to the bus. This will return a value from specified address starting
	 * with the module with lowest index (main memory preferably). If there's any address
	 * space overlap, the module with lower index will be read, but all modules do get the
	 * read command.
	 *
	 * @param addr
	...
	public synchronized Object read(long addr)
	{
		...
		for (int i = bus_modules.size() - 1; i >= 0; i--)
		{
			...
			if (addr >= module.startAddr() && addr <= module.endAddr())
			{
				if (!module.phantom)
				{
	...
	/**
	 * Issue a write to the bus. The write will be issued to all modules if there's an
	 * address overlap.
	 *
	 * @param addr
	 *            Address to write to
	 ...
	 *            Data to write
	 * @param isInstr
	 *            Specify whether the data is instruction or not
	...
	public synchronized int write(long addr, Object data, boolean isInstr)
	{
		...
		Msg.D("Writing " + String.format("0x%08x", ((Long) data)) + " to "
				+ String.format("0x%08x", addr), 5, this);
	...
	/**
	 * Check if the specified address is mapped to a module AND it is initialized.
	 *
	 * @param addr
	 *            Address to check
	...
	public int eval(int index)
	{
		...
		{
			return Msg.E("eval(" + index + "): error: " + e, Constants.PLP_SIM_BUS_ERROR,
					this);
		}
	}
	 ...
	 *            Index of the module
	 * @param x
	 *            Object to draw to, left to the module to implement
	 * @return PLP_OK, or error code
	 */
	...
	/**
	 * Disable all I/O mods. Including the main memory if it is attached to the bus. Be
	 * careful.
	 *
	 * @return Returns 0 on completion.
	 */
	...
	public synchronized boolean getEnabled(int index)
	{
		...
		{
			Msg.E("enabled(" + index + "): error: " + e, Constants.PLP_SIM_BUS_ERROR,
					this);
	...
	/**
	 * Clear saved values of specified module. This renders all registers of the module to
	 * be marked uninitialized
	 *
	 * @param index
	 *            Index of the module
	 * @return PLP_OK, or error code
	...
	public String introduceMod(int index)
	{
		...
		{
			Msg.E("introduceio(" + index + "): error: " + e, Constants.PLP_SIM_BUS_ERROR,
					this);
	...
	public synchronized long getModStartAddress(int index)
	{
		...
		{
			Msg.E("iostartaddr(" + index + "): error: " + e, Constants.PLP_SIM_BUS_ERROR,
					this);
	...
	public synchronized long getModEndAddress(int index)
	{
		...
		{
			Msg.E("ioendaddr(" + index + "): error: " + e, Constants.PLP_SIM_BUS_ERROR,
					this);
	...
	public synchronized PLPSimBusModule getRefMod(int index)
	{
		...
		{
			Msg.E("getRefMod(" + index + "): error: " + e, Constants.PLP_SIM_BUS_ERROR,
					this);
	...
	public synchronized void issueZeroes(int index)
	{
		...
		{
			this.write(module.startAddr() + i * (module.isWordAligned() ? 4 : 1),
					new Long(0), false);
		}
	}
