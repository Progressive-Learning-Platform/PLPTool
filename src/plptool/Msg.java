...
public class Msg
{
	...
	{
		if (suppressWarnings || LogOutStream == null)
			return;
		...
		try
		{
			...
			if (objIdentifier != null)
				if (output == null)
					LogOutStream.println("[WARNING] " + objIdentifier.toString() + ": "
	...
	public static void I(String infoStr, Object objIdentifier)
	{
		...
		try
		{
			...
			if (objIdentifier != null)
				if (output == null)
					LogOutStream.println(objIdentifier.toString() + ": "
							+ infoStr.replaceAll("<[^>]*>", ""));
				else
				{
					kit.insertHTML(doc, doc.getLength(),
	...
	public static void D(String debugStr, int requestedDebugLevel, Object objIdentifier)
	{
		...
		if (requestedDebugLevel <= Constants.debugLevel)
		{
			try
			{
				...
				if (objIdentifier != null)
				{
					LogOutStream.println("[DEBUG] " + objIdentifier.toString() + ": "
							+ debugStr);
					if (output != null)
					{
						kit.insertHTML(
	...
	public static void P(String printStr)
	{
		...
		try
		{
			...
			{
				kit.insertHTML(doc, doc.getLength(),
						"<pre><font face=monospaced size=10pt>" + printStr
								+ "</font></pre>", 0, 0, null);
				output.setCaretPosition(doc.getLength());
			}
	...
	public static void P()
	{
		...
		try
		{
			...
			{
				kit.insertHTML(doc, doc.getLength(),
						"<pre><font face=monospaced size=10pt>" + p + "</font></pre>", 0,
						0, null);
				output.setCaretPosition(doc.getLength());
			}
			p = "";
	...
	public static void mark()
	{
		...
		try
		{
			...
			else
			{
				kit.insertHTML(doc, doc.getLength(),
						"<b><font color=gray>[DEBUG]</font></b> "
								+ ": <font color=darkgray>mark: " + markCounter
								+ "</font><br />", 0, 0, null);
				output.setCaretPosition(doc.getLength());
			}
