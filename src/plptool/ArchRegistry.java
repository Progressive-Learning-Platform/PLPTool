...
/**
 * This class associates an ISA implementation to its member classes. Developers porting
 * an ISA to PLP Tool framework will have to register their implementation here.
 *
 * @author wira
 */
public class ArchRegistry
{
	private static HashMap<Integer, Class> archClasses = new HashMap<Integer, Class>();
	private static HashMap<Integer, String> archIdentifiers = new HashMap<Integer, String>();
	private static HashMap<Integer, String> archDescriptions = new HashMap<Integer, String>();
	...
	/**
	 * Return the specified architecture meta class. Register your ISA meta class here
	 * (extension of plptool.PLPArchitecture).
	 *
	 * @param index
	 */
	public static PLPArchitecture getArchitecture(ProjectDriver plp, int ID)
	{
		...
		try
		{
			...
			if (archClass == null)
			{
				Msg.E("Invalid ISA ID: " + ID, Constants.PLP_ISA_INVALID_ARCHITECTURE_ID,
						null);
				return null;
			}
	 ...
	 *
	 * @param arch
	 *            Architecture class that extends plptool.PLPArchitecture
	 * @param ID
	 *            Integer ID for the ISA
	 * @return PLP_OK if the class is successfully registered, error code otherwise
	 */
	public static int registerArchitecture(Class arch, int ID, String strID,
			String description)
	{
		Msg.D("Registering ISA class " + arch.getCanonicalName() + " with ID=" + ID
				+ " strID=" + strID, 2, null);
		...
		if (!arch.getSuperclass().getCanonicalName().equals("plptool.PLPArchitecture"))
			return Msg.E("Specified class does not extend the PLP "
					+ "architecture superclass.", Constants.PLP_ISA_INVALID_METACLASS,
					null);
