...
/**
 * This class is the dynamic module loading framework for PLPTool. The class loader and
 * the class load routine are generic, they can be used to load generic classes and attach
 * them to the list to be used in plptool later for any reason. Object instantiations of
 * dynamic classes are superclass specific. Instantiation routines are expected to return
 * a null reference if they failed and call error #1040 (e.g. typecasting error).
 *
 * @author wira
 */
public class DynamicModuleFramework
{
	...
	/**
	 * Module class loading routine for PLPTool dynamic module framework. This function is
	 * called by PLPToolApp class during program startup via the --load-class parameter.
	 *
	 * @param path
	 *            Path of the Java class file
	 * @param className
	 *            Java class name (package.subpackage...class)
	...
	public static boolean loadModuleClass(String path, String className)
	{
		if (!warned)
		{
			Msg.W("YOU ARE LOADING A DYNAMIC MODULE, THIS COULD POTENTIALLY BE DANGEROUS",
					null);
	...
	/**
	 * Check whether the specified module class is registered and return the index of the
	 * class in the list
	 *
	 * @param name
	...
	/**
	 * Instantiate a dynamic module and add a reference to it in the dynamicInstances
	 * list.
	 *
	 * @param index
	 *            Index of the class
	...
	public static int newModuleInstance(int index)
	{
		...
		{
			Msg.E("Instantiation exception for module "
					+ getDynamicModuleClass(index).getName() + ". "
		...
		{
			Msg.E("Illegal access exception for module "
					+ getDynamicModuleClass(index).getName(),
	 ...
	 *
	 * @param index
	 *            Index of the module object to return
	...
	public static Object getModuleInstance(int index)
	{
		...
		{
			Msg.E("Invalid index.", Constants.PLP_DMOD_INVALID_MODULE_INDEX, null);
			return null;
		}
	 ...
	 *
	 * @param className
	 *            Class name of the module to check
	 * @return True if the framework determines that an instance is already loaded, false
	 *         otherwise
	 */
	 ...
	 *
	 * @param index
	 *            Index of the object reference in the list
	...
	/**
	 * Autoload modules from the specified directory and apply manifest files to the
	 * provided ProjectDriver instance. If prompt is true, this method will prompt the
	 * user for module autoload.
	 *
	 * @param autoloadPath
	 * @param plp
	 * @param prompt
			...
			plptool.gui.ProjectDriver plp, boolean prompt)
	{
		...
		if (autoloadDir.exists() && autoloadDir.isDirectory())
		{
			...
			if (files.length > 0)
			{
				...
				if (plp.g() && prompt)
				{
					int ret = JOptionPane.showConfirmDialog(plp.g_dev, message,
							"Autoload Modules", JOptionPane.YES_NO_OPTION);
		...
		if (autoloadDir.exists() && autoloadDir.isDirectory())
		{
			...
			if (autoloadDir.exists())
				Msg.E("Failed to remove autoload directory. "
						...
						+ "option from the command line.", Constants.PLP_GENERIC_ERROR,
						null);
		}
	}
	 ...
	 * @param index
	 *            Index of the class
	 * @return Reference to the newly instantiated object casted to a simulation bus
	 *         module. Returns null pointer if instantiation failed for any reason.
	 */
	public static PLPSimBusModule newBusModuleInstance(int index)
	{
		...
		{
			Class moduleClass = getDynamicModuleClass(index);
			return moduleClass == null ? null : (PLPSimBusModule) moduleClass
					.newInstance();
	...
	/**
	 * Load classes from a JAR file that contains the plp.manifest class listing file
	 *
	 * @param path
	 *            Path to the JAR file
	 ...
	 *
	 * @param jar
	 *            Path to jar file that contains the manifest
	 * @param manifestLines
	 *            Manifest lines returned by loadJarWithManifest
	 * @param plp
	 *            Reference to the ProjectDriver to apply the manifest to
	 */
	...
	/**
	 * Generate a plp.manifest file for a given Java package as specified by the path.
	 * This method will recursively go through the directories, resolve
	 * interface/superclass dependencies and generate the manifest file.
	 *
	 * @param path
	 *            Path to Java package directory
	 * @param connectorClass
	 *            Class implementing PLPGenericModule that will be loaded by PLPTool when
	 *            the module is loaded
	 * @return String of the plp.manifest file
	 */
	public static String generateManifest(String path, String title, String author,
			String license, String description, String version)
	{
		String manifest = "";
		String t;
		...
		if (packageDir.getName().endsWith(".jar"))
		{
			...
			{
				Msg.E("Manifest generation failed.", Constants.PLP_GENERIC_ERROR, null);
				Msg.trace(e);
				return null;
			}
		}
		...
		{
			Msg.E("'" + path + "' does not exist.", Constants.PLP_GENERIC_ERROR, null);
			return null;
		}
		else if (!packageDir.isDirectory())
		{
			...
			generateManifestTraverseDirectory(packageDir, packageDir, classes,
			...
			if (ascending)
			{
				...
				for (i = 0; i < classes.size(); i++)
				{
					if (!(isModuleClassRegistered(classNames.get(i)) != -1)
							&& loadModuleClass(classes.get(i).getAbsolutePath(),
									classNames.get(i)))
					{
		...
		for (i = 0; i < dynamicModuleClasses.size(); i++)
		{
			...
			{
				if (interfaces[j].getCanonicalName().equals(
						"plptool.dmf.ModuleInterface5"))
				{
			...
			if (dynamicModuleClasses.get(i).getSuperclass().getCanonicalName()
					.equals("plptool.PLPArchitecture"))
			{
	...
	/**
	 * Helper recursive function for generateManifest. This function populates the classes
	 * list from the specified directory
	 *
	 * @param path
	 *            Path to recurse into
	 * @param classes
	 *            Array list of classes
	 */
	private static void generateManifestTraverseDirectory(File path, File rootPath,
			ArrayList<File> classes, ArrayList<String> classNames,
			ArrayList<String> resources)
	{
		...
		for (int i = 0; i < list.length; i++)
		{
			...
			else if (list[i].isFile() && list[i].getName().endsWith(".class"))
			{
				...
				classes.add(list[i]);
				className = list[i].getAbsolutePath().substring(
						rootPath.getAbsolutePath().length() + 1);
	 ...
	 *
	 * @param extension
	 *            File extension (with period, e.g. ".png")
...
/**
 * This class contains the handlers for manifest entries that the module framework
 * supports
 *
 * @author wira
 */
class ManifestHandlers
{
	...
	public static void m_loadmodule5()
	{
		...
		if (cIndex > -1)
		{
			...
			ModuleInterface5 mod = (ModuleInterface5) DynamicModuleFramework
					.getModuleInstance(cIndex);
			...
			else
			{
				...
				{
					if (plp.g())
						PLPToolbox.showErrorDialog(plp.g_dev,
	...
	public static void m_extracttotemp()
	{
		...
		PLPToolbox.copyFromJar(jar, tokens[1], PLPToolbox.getTmpDir() + "/" + tokens[2]);
	}
...
class PLPDynamicModuleClassLoader extends ClassLoader
{
	 ...
	 *
	 * @param parent
	 *            Parent classloader, preferably top level
	 */
	...
	/**
	 * Set the path of the class file and the name of the class to load when loadClass
	 * routine is called
	 *
	 * @param path
			...
			NoClassDefFoundError
	{
		...
		try
		{
			...
			else if (fName.endsWith(".jar"))
			{
				...
				{
					Msg.E("defineClass: " + name + " FAILED: " + e,
							Constants.PLP_GENERIC_ERROR, null);
					jar.close();
					return null;
				}
